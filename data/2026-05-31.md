...
TL;DR: 提出了一种基于机器学习的Phast方法，利用共享波变换编码器和双分支结构（计数分支+时间分支），同时从PMT波形中重建光电子计数和时间分布，在多种模拟数据集上表现出稳定准确的性能。


<details>
  <summary>Details</summary>
Motivation: 在实际探测器环境中，PMT波形重建受到堆积、电荷涨落、噪声等电子学效应的影响，传统方法难以精确恢复光电子数和时间等物理观测量。

Method: 采用共享波变换编码器（wave-transformer encoder），后接两个专用分支：计数分支用于预测总PE数，时间分支采用计数条件查询解码器（count-conditioned query decoder）并支持动态查询激活。

Result: 在均匀和混合快慢双时间分量配置的玩具蒙特卡洛PMT波形数据集上，该方法在PE计数和时间重建方面均表现出稳定、准确且高度一致的性能。

Conclusion: 卷积特征提取与基于查询的变换解码器相结合的架构为复杂PMT波形重建任务提供了有效途径。
...
TL;DR: 本文利用Belle探测器在KEKB对撞机上采集的1.02亿Υ(1S)和1.58亿Υ(2S)衰变样本，搜索Ξ⁰p、Ω⁻p和Ω⁻n双重子态，未发现显著信号，首次在90%置信水平上设定这些衰变分支比的上限，量级为O(10⁻⁷)-O(10⁻⁶)。


<details>
  <summary>Details</summary>
Motivation: 多奇异重子-重子相互作用与中子星物质性质密切相关，但目前缺乏实验约束。此前理论和实验研究表明ΞN和ΩN相互作用可能存在吸引力，这促使研究者搜索可能存在的弱束缚态（双重子态），以加深对强相互作用和致密天体物质的理解。

Method: 利用Belle探测器在KEKB非对称能量e⁺e⁻对撞机上采集的1.02亿Υ(1S)和1.58亿Υ(2S)衰变事件样本，在相应重子对阈值附近的质量区域搜索Ξ⁰p、Ω⁻p和Ω⁻n双重子态信号，通过统计显著性检验并设定分支比上限。

Result: 在所有搜索通道中均未观察到显著信号。首次在90%置信水平上给出Υ(1S)和Υ(2S)衰变到Ξ⁰p、Ω⁻p和Ω⁻n双重子态的分支比上限，其量级随通道和假设质量差不同而变化，范围在O(10⁻⁷)-O(10⁻⁶)。

Conclusion: 未能确认Ξ⁰p、Ω⁻p和Ω⁻n双重子态的存在，但通过设定严格的上限为未来相关实验搜索提供了重要参考，有助于约束多奇异重子-重子相互作用的理论模型。
...
TL;DR: 本文从参数依赖常微分方程的角度，研究了Jost函数在复能量平面上的解析性质。通过因子化动量相关的分支项，将散射问题转化为系数为单值解析函数的微分系统，证明了变换后的Jost函数在有限径向距离上是能量的单值解析函数。


<details>
  <summary>Details</summary>
Motivation: Jost函数的解析性质是量子散射理论及散射矩阵解析延拓的基础，但其在复能量平面上的多值依赖性的数学根源需要从常微分方程参数依赖的角度进行严格刻画。

Method: 从短程中心势的径向薛定谔方程出发，推导出与Riccati-Bessel和Riccati-Neumann解相关的系数函数的一阶微分系统。通过显式因子化动量相关的分支项，将散射问题转换为系数为复能量单值解析函数的微分系统。利用常微分方程解对参数的解析依赖经典理论进行分析。

Result: 证明了变换后的Jost函数在有限径向距离上是能量变量的单值解析函数；揭示了能量-动量平方根关系是Jost函数多值依赖性的根源；讨论了因子化过程的几何解释（与黎曼面拓扑的关系）。

Conclusion: 通过因子化分支项，成功将Jost函数的解析性转化为标准常微分方程参数依赖问题，为散射矩阵的解析延拓提供了严格的数学基础。
...
TL;DR: 本文以实心球在斜面上无滑滚动为例，详细回顾和分析了带约束哈密顿系统的狄拉克量子化方法，重点讨论了从经典到量子力学的过渡中狄拉克-伯格曼算法和狄拉克括号的作用，并证明将哈密顿算符限制到物理希尔伯特子空间可得到与内禀方法一致的薛定谔方程。


<details>
  <summary>Details</summary>
Motivation: 通过一个简单但丰富的物理实例（球在斜面上无滑滚动），系统阐述并验证带约束哈密顿系统的狄拉克量子化方法的正确性和一致性。

Method: 采用狄拉克-伯格曼算法处理完整约束和非完整约束，将经典狄拉克括号替换为量子对易子，并通过将哈密顿算符投影到物理希尔伯特子空间实现约束的量子化。

Result: 证明了约束系统的哈密顿算符限制到物理希尔伯特子空间后，所导出的薛定谔方程与内禀方法得到的方程完全一致。

Conclusion: 该结果进一步证实了狄拉克量子化方法在处理带约束哈密顿系统时的自洽性和有效性。
...
TL;DR: 本文建立了一个系统的框架，用于分析受倒谐振子势 $-rac{1}{2} m^2\omega^2 x^2$ 作用的Klein-Gordon标量场的量子和热力学性质。通过非厄米动量替换和辛相空间旋转，将系统映射到可解析求解的有效谐振子，并推导出配分函数、自由能、熵和热关联函数的闭合形式表达式。该框架应用于宇宙暴胀、黑洞视界附近和凝聚态相变三种物理场景，统一了文献中的分散结果，并对不稳定量子系统的有限温谱密度和纠缠熵给出了新预测。


<details>
  <summary>Details</summary>
Motivation: 倒谐振子势在 cosmology（暴胀涨落）、黑洞物理（视界附近的量子场）和凝聚态（二级相变序参量动力学）等跨领域问题中频繁出现，但缺乏统一的理论框架来系统处理其量子和热力学性质。

Method: 采用非厄米动量替换 $P 	o P - m\omega x$，再通过辛相空间旋转 $V = \exp\left[-\frac{\pi}{8}(xp+px)\right]$ 将系统映射到有效谐振子系统（坐标变为 $xe^{i\pi/4}$），从而定义良定义的配分函数并推导出自由能、熵和热关联函数的闭合形式表达式。

Result: 获得了配分函数 $Z(\beta,\omega,m)$ 的闭合形式表达式，以及自由能、熵和热关联函数的解析结果。将该框架应用于宇宙暴胀、黑洞视界附近和凝聚态二级相变三种场景，统一了此前文献中的分散结果，并给出了有限温谱密度和纠缠熵的新预测。

Conclusion: 本文建立的辛相空间旋转方法为不稳定量子系统（倒谐振子）提供了统一的量子和热力学分析框架，揭示了不同物理场景（宇宙暴胀、黑洞物理、凝聚态相变）之间的深层数学联系，并产生了可检验的新物理预测。
...
TL;DR: 本文提出利用时空纠缠光子对和符合探测后选择技术，在弱散射介质中实现弹道光子与散射双光子的分离，从而提高图像对比度，并通过数值模拟和实验验证了该方法在两种配置下的有效性。


<details>
  <summary>Details</summary>
Motivation: 传统方法（时间门控、空间滤波）在自适应光学、时间门控和空间滤波不可行的场景中存在局限性，需要一种基于量子成像框架的新策略来提升散射介质中的成像对比度。

Method: 利用时空纠缠光子对照射目标，通过符合探测（coincidence detection）和空间关联后选择（post-selection on spatially correlated events）来隔离弹道光子与散射双光子，从而增强图像对比度，同时消除背景光事件。

Result: 数值模拟和实验（两种配置：双光子均照射场景、仅单光子照射场景）均表明，关联后选择能提升图像对比度，但代价是较高散粒噪声（事件数减少）；可通过合并多个后选择窗口的事件来部分补偿。

Conclusion: 该量子成像方法可在传统滤波技术不适用时扩展散射介质成像的应用范围，为量子成像框架下的散射介质成像提供新途径。
...
TL;DR: 该论文探索了一种针对超弹性材料非线性弹性问题的变分量子公式化方法，旨在适用于近期的含噪中等规模量子（NISQ）设备。该方法利用超弹性的势能结构，采用混合量子-经典框架，通过参数化量子电路评估能量泛函，并通过经典优化进行求解。为实现量子硬件部署，引入了应变能密度的多项式近似。以一维NeoHookean材料模型为例，采用有限元离散化进行数值实验，验证了该方法的可行性。


<details>
  <summary>Details</summary>
Motivation: 当前非线性弹性问题的数值求解计算成本高，而NISQ量子设备有望提供加速。论文旨在探索一种适用于近期量子硬件的变分量子方法，解决超弹性材料模型中的非线性弹性问题。

Method: 采用混合量子-经典框架：利用参数化量子电路（PQC）评估超弹性势能泛函，通过经典优化器更新电路参数。引入应变能密度的多项式近似，使能量函数形式与变分量子算法兼容。以一维NeoHookean材料为例，采用一阶和二阶形函数的有限元离散化，施加非均匀边界条件。

Result: 数值实验表明，所提出的方法能够在NISQ设备上实现非线性弹性问题的求解；多项式近似阶数对求解精度和效率有显著影响，选择合适的阶数可在保持精度的同时降低量子资源开销。

Conclusion: 该变分量子公式化方法具有在近期量子设备上求解超弹性非线性弹性问题的可行性，为量子计算在计算力学中的应用提供了新的途径。
...
TL;DR: 本文展示了氢终端金刚石表面的吸附受主与氮替代施主形成光学活性施主-受主对(DAP)，通过插入本征CVD中间层可精确调节施主-受主间距，实现纳米级可调的单光子发射器。


<details>
  <summary>Details</summary>
Motivation: 探究氢终端金刚石表面受主吸附层与体相氮施主之间是否形成光学活性施主-受主对，并建立可精确调控的量子发射平台。

Method: 在富氮HPHT金刚石衬底与氢终端表面之间插入不同厚度的本征CVD中间层，以纳米精度调控施主-受主间距；通过光致发光光谱、时间分辨荧光和单光子统计等手段表征DAP复合发光特性。

Result: 发现随中间层厚度变化，DAP复合发光呈现明亮的窄线光谱，其强度、能量和衰减动力学系统变化；单根发射线表现出单光子统计特性，整体系综展现强偏振各向异性。

Conclusion: 发现了氢诱导表面转移掺杂在金刚石中的光学对应物，建立了表面定义的、纳米可调的DAP量子发射器平台。
...
TL;DR: 本文研究了极化保真度、Holevo基和广义保真度的酉因子，证明了极化路径实现了从FM到FU的区间，并分类了Holevo基以及可来自基R的酉因子。


<details>
  <summary>Details</summary>
Motivation: 受Afham-Ferrie广义保真度理论中若干开放问题的驱动，旨在深入理解极化保真度的性质及Holevo基的分类。

Method: 在复正定矩阵锥\(\Pd\)上，通过分析单边极化保真度\(x\mapsto F_{P^x}(P,Q)\)和\(x\mapsto F_{Q^x}(P,Q)\)及其对称化形式的单调性，证明其在\((-\infty,1]\)上非减、在\([1,\infty)\)上非增；求解固定对的Holevo基方程\(F_R(P,Q)=\FH(P,Q)\)；分类所有Holevo基及可来自基R的酉因子（条件为\(P^{-1/2}Q^{1/2}W\)相似于正定矩阵）。

Result: 极化路径精确实现区间\([\FM(P,Q),\FU(P,Q)]\)在\([-1,1]\)上；对于\(z\ge1/2\)可逐点点态实现所有z-保真度值及Log-Euclidean保真度；当\(d\ge2\)且\(0<z<1/2\)时一般无法实现（即使内部保真度）；分类了所有Holevo基；证伪了全局反向包含\(SU(d)\)（对\(d\ge2\)）。

Conclusion: 极化保真度提供了广义保真度区间实现的精确刻画，Holevo基与酉因子的分类揭示了广义保真度代数结构的完整特征。
...
TL;DR: 本文发现光阱中的反冲加热速率并非如当前研究假设的等于自由空间值，而是可以通过Purcell效应在微腔中显著抑制，并为此建立了基于宏观量子电动力学和少模量子化方法的通用理论框架。


<details>
  <summary>Details</summary>
Motivation: 光散射引起的反冲加热是光阱中退相干的基本来源，严重限制了非经典运动态的实现。在相干散射构型的腔系统中，由于尖锐光学共振的存在，传统微扰方法失效，缺乏预测反冲加热率的理论，当前工作假设该速率近似等于自由空间值。

Method: 基于宏观量子电动力学和纳米光子学中发展的少模量子化方法，建立适用于任意电磁结构下 trapped particles 的通用理论框架。

Result: 电磁环境可通过Purcell效应强烈改变反冲加热率，在先进微腔中，质心运动和转动运动的加热率均可被显著抑制。

Conclusion: 该理论框架为通过光子结构设计工程化悬浮光力学中的运动退相干提供了途径。
...
TL;DR: 本文研究了通过玻色子环境相互作用的两个自旋系统的时间演化，分析了不同环境模型下负性度作为纠缠度量的行为，得到了实现最大自旋纠缠的环境参数条件，并推导出在退相干可忽略情况下的有效酉算符。


<details>
  <summary>Details</summary>
Motivation: 量子计算的发展需要有效实现量子计算的物理模型，而环境在系统中既导致退相干又通过自旋间相互作用诱导纠缠，因此研究环境对量子系统行为的影响对于有效实现量子信息器件至关重要。

Method: 考虑两个自旋通过玻色子环境相互作用的时间演化，以负性度作为纠缠度量，对不同环境模型下自旋间的纠缠行为进行数值或解析研究，分析环境参数对纠缠和退相干的影响。

Result: 获得了实现自旋间最大纠缠值所需的环境参数条件，找到了能在最小化系统退相干的同时最大化其纠缠的环境模型，并在退相干影响可忽略的条件下推导出了描述相应演化的有效酉算符。

Conclusion: 通过选择合适的环境模型和参数，可以在抑制退相干的同时增强自旋间的纠缠，从而为量子信息器件的有效实现提供理论指导。
...
TL;DR: 本文提出了一种基于机器学习的隧穿相图框架，通过解码KIE与隧穿因子κ之间的非线性关系，成功解耦了量子隧穿的真实强度，并在300-600 K范围内发现了高KIE-低κ的异常区域，为量子隧穿的定量评估提供了新范式。


<details>
  <summary>Details</summary>
Motivation: 传统KIE作为量子隧穿探针存在固有限制，其复合性质将隧穿效应与零点能和经典动力学混杂在一起，无法真实反映隧穿强度。

Method: 引入隧穿相图这一机器学习框架，通过解码KIE与隧穿因子κ之间的非线性关系来解耦真实的隧穿强度。

Result: 该框架实现了高保真度（R² > 0.98, RMSE = 0.21），并在300-600 K温度范围内发现了一个反常的高KIE-低κ区域。

Conclusion: 该工作为量子隧穿的定量评估定义了一个新范式，克服了传统KIE作为隧穿探针的局限性。
...
TL;DR: 本文推导了位置空间与动量（波矢）空间中方差之间的尖锐不确定关系，其形式均为ΔrΔk≥5/2，并证明该关系在经典光束理论、量子相干光束理论以及单光子量子理论中具有相同形式。


<details>
  <summary>Details</summary>
Motivation: 旨在建立一种比传统海森堡不确定性原理更精确（尖锐）的不确定关系，并检验其在经典光学和量子光学中的统一性。

Method: 通过理论推导，分别从经典光束理论、量子相干光束理论以及单光子量子理论出发，推导位置空间与波矢空间方差的不确定关系。

Result: 得到了统一的尖锐不确定关系ΔrΔk≥5/2，该下界比海森堡不确定关系（ΔxΔp≥ħ/2）的对应形式更紧。

Conclusion: 该不确定关系在经典与量子光学中具有普适性，表明位置-波矢的不确定度下界为5/2，超越了传统的不确定关系极限。
...
TL;DR: 本文提出利用明亮压缩真空光与单次正交测量，无需弱耦合条件下的条件准备即可超快生成物质中的宏观量子态（如Dicke态和猫态），为强场量子光工程化宏观量子物质开辟了新途径。


<details>
  <summary>Details</summary>
Motivation: 现有的弱耦合机制中，多光子量子光与物质纠缠后，物质态无条件地退化为经典混合态，无法高效制备宏观量子态。本文旨在通过基于正交测量的后选择方案克服这一限制。

Method: 采用明亮压缩真空光与共振电偶极耦合的二能级系统系综相互作用，结合单次正交测量进行后选择（heralding）。该后选择机制表现为对电极化的高斯滤波器，通过调整压缩光亮度加速制备零本征值Dicke态，并利用反旋项驱动从Dicke态到猫态的随机转变。

Result: 证明了在弱耦合下，通过正交测量后选择可在物质中制备高斯加权的量子叠加态；更亮的压缩真空光可加速Dicke态的制备；反旋项进一步实现了从Dicke态到类猫态的随机转变。

Conclusion: 该工作展示了利用强场量子光实现超快工程化宏观量子物质的可能性，为量子光学与物质量子态的制备提供了新方法。
...
TL;DR: 本文提供了量子多体系统中强对称性及其自发破缺为弱对称性的局部化理解，引入无限体积下强对称性的严格定义，并提出介于强弱对称性之间的冯·诺依曼对称性概念，导出了其Lieb-Schultz-Mattis型反常约束。


<details>
  <summary>Details</summary>
Motivation: 先前对"强到弱对称性破缺"的理解本质上是一种非局域效应，缺乏局域化的诊断工具。本文旨在建立一种显式局域的方式来理解和诊断强对称性及其破缺。

Method: (1) 给出无限体积极限下强对称性的严格定义，推广有限体积定义；(2) 提出多种等价表述，包括"局域电荷相干性"概念；(3) 引入冯·诺依曼对称性作为介于强、弱对称性之间的新对称性类型；(4) 推导量子自旋链中冯·诺依曼对称性的Lieb-Schultz-Mattis型反常约束。

Result: 建立了强对称性的局域诊断框架，给出了无限体积下强对称性的严格数学定义及其等价形式，发现了冯·诺依曼对称性这一中间对称性类型。

Conclusion: 强对称性及其自发破缺可以在局域层面得到严格理解和诊断；冯·诺依曼对称性在无限体积系统中作为中间对称性类型出现，并受到Lieb-Schultz-Mattis型反常约束。
...
TL;DR: 本文提出了一种基于分步法和模式光场表示的解析框架，用于描述空间光模式在湍流大气中的传播。该框架直接量化了湍流引起的相位波动如何将原始模式的功率耗散并重新分配到相邻空间模式，且功率转移在均匀信道中与传播距离呈线性关系，可通过矩阵指数形式求解任意距离的简单解。转移速率由湍流谱与相互作用模式对的接收谱之间的空间谱重叠决定。该模型在单一模式主导时精确，预测与中到强湍流水平下的模拟结果吻合良好，并验证了之前基于经验观察的模式阶数缩放规律。


<details>
  <summary>Details</summary>
Motivation: 传统描述空间光模式在湍流大气中传播的方法复杂且缺乏解析可解性，难以直接揭示模式间功率转移的物理机制及定量规律。

Method: 基于分步法（split-step approach）和模式光场表示，推导湍流诱导相位波动导致模式间功率转移的解析表达式。利用均匀信道中功率转移与距离的线性关系，通过矩阵指数形式获得任意传播距离的解析解。转移速率由湍流谱与模式对接受谱的空间谱重叠积分决定。

Result: 模型在单一模式主导时精确，与中到强湍流水平下的数值模拟结果吻合良好，并成功复现了之前仅由经验观测得到的模式阶数缩放规律。

Conclusion: 该解析框架为理解空间光模式在湍流大气中的传播行为提供了简洁而有力的工具，尤其适用于光通信和成像系统中的模式串扰分析与系统设计。
...
TL;DR: 本文研究了量子拓扑数据分析（QTDA）中，哪些直接作用于经典数据集的量子编码能够最好地保留原始数据的拓扑不变量，区别于传统QTDA从组合对象（如单纯复形）出发的方法，首次从量子编码的视角探讨直接提取拓扑特征的可行性。


<details>
  <summary>Details</summary>
Motivation: 传统量子TDA方法通常从单纯复形等组合对象出发，这些对象的构建本身需要大量资源。本文希望探索更直接的路径：直接对经典数据集进行量子编码，并研究哪些编码能保留原始数据的拓扑不变量，从而为量子算法直接提取拓扑特征奠定基础。

Method: 论文比较了不同的量子编码方案（如振幅编码、角度编码等），分析它们对原始数据拓扑不变量的保持能力。通过考量变换对同调群等拓扑不变量的影响，确定哪些编码方式在量子计算框架下是"允许的"（admissible）。

Result: 识别出了几类能有效保留原始数据拓扑不变量的量子编码方法，并给出了编码允许性的判定准则，为直接对经典数据应用量子TDA算法提供了理论依据。

Conclusion: 本文开启了一条新的研究路径：从数据本身的量子编码出发进行拓扑数据分析，绕过传统单纯复形构建的瓶颈。所提出的编码允许性框架有助于指导未来量子TDA算法的设计与实现。
...
TL;DR: 本文对硅自旋量子比特平台上高保真逻辑魔法态的生成进行了资源分析，比较了多种架构（SpinBus、密集最近邻、混合方案）、纠错码（表面码、颜色码、偏置纠错码）和蒸馏协议（5→1、15→1），结合自底向上和自顶向下方法，发现优化控制脉冲可减少42%的蒸馏开销，硅定制偏置纠错码相比表面码可减小约三倍物理 footprint。


<details>
  <summary>Details</summary>
Motivation: 在硅自旋量子比特平台上实现容错量子计算需要高效生成高保真逻辑魔法态，而目前缺乏系统性的资源分析框架来评估不同架构、纠错码和蒸馏策略的组合效果。

Method: 采用自底向上方法：基于硅处理器哈密顿量构建包含1/f非马尔可夫噪声的硬件级噪声模型，结合控制脉冲优化；采用自顶向下方法：设定目标逻辑保真度推导硬件性能约束。分析了三种架构、三种纠错码和两种蒸馏协议。

Result: 优化控制脉冲使魔法态蒸馏开销降低42%；硅定制偏置纠错码在无需物理偏置保持操作的情况下，相比表面码实现约三倍物理 footprint缩减。

Conclusion: 该框架可系统评估资源缩减策略，为硅基自旋量子比特平台的容错量子计算架构设计提供定量指导。
...
TL;DR: 量子力学在排列信道中实现了显著的零错误通信优势：对于循环重排，经典消息数量为$d^n/n$，而量子协议可恢复$d^n$，辅助量子协议更达到$d^{2n}/n$；对于完全扰乱的对称群$S_n$，经典消息为$n^{d-1}$，量子协议为$n^{d(d+1)/2-1}$，辅助量子协议为$n^{d^2-1}$。


<details>
  <summary>Details</summary>
Motivation: 探索量子力学在位置不确定的排列信道中能否实现超越经典理论的零错误通信能力。

Method: 通过分析循环群、二面体群、对称群等排列群下的经典与量子零错误通信容量，推导了类似Pólya计数公式的可区分消息数公式，并考虑辅助量子比特的密集编码协议。

Result: 量子协议在循环重排下可恢复身份信道容量$d^n$（经典为$d^n/n$），辅助量子协议达$d^{2n}/n$（实现密集编码）；对称群下量子协议为$n^{d(d+1)/2-1}$（经典为$n^{d-1}$），辅助量子协议为$n^{d^2-1}$。

Conclusion: 量子力学在位置不确定的通信和存储场景中提供了根本性的零错误通信优势。
...
TL;DR: 本文提出了在量子设备上对一组重要衍生品进行定价的方法，相较于现有经典方法具有优势，并且在定价某些路径依赖型期权合约时，相比最先进的量子蒙特卡洛方法，在量子比特需求上实现了指数级优势。


<details>
  <summary>Details</summary>
Motivation: 现有经典方法在衍生品定价上存在计算效率瓶颈，而量子方法虽具有潜力，但在路径依赖型期权等复杂合约上仍面临量子资源（如量子比特数）过高的挑战。本文旨在开发在量子设备上定价衍生品的新方法，在获得对经典方法优势的同时，显著降低量子资源开销。

Method: 本文结合GumaroS2026的工作，提出了一套适用于量子设备的衍生品定价方法体系，重点针对路径依赖型收益的期权合约进行优化，通过创新的量子线路设计和算法架构，在保持量子加速的同时大幅压缩量子比特需求。

Result: 所提出的方法对一组重要衍生品定价时优于现有经典方法；在与最先进的量子蒙特卡洛方法对比中，针对路径依赖型期权定价，在量子比特数需求上实现了指数级（exponential advantage）的降低。

Conclusion: 本文成功实现了在量子设备上对衍生品的高效定价，既获得了对经典方法的优势，又在量子资源（尤其是量子比特数）上取得了对已有量子方法的指数级改进，为量子金融领域提供了更实用的路径依赖型期权定价方案。
...
TL;DR: 本文基于纠缠动力学在IBM量子处理器上实现了素数识别的量子协议，通过提取纠缠时间演化中的特定傅里叶分量来区分素数与非素数，并引入全局重缩放因子的噪声抑制方法，理论上给出了新的解析界以增强区分度，展示了在NISQ设备上实现数论应用的可行性。


<details>
  <summary>Details</summary>
Motivation: 探索在近期含噪量子设备（NISQ）上实现实用的数论应用，特别是利用量子纠缠动力学进行素数识别，克服实验噪声对量子协议精度的影响。

Method: 将整数的素性与其在双体量子系统中纠缠时间演化的特定傅里叶分量相关联；引入基于全局重缩放因子的噪声缓解方法，在校准子集电路后外推至不同配置；理论上推导了初始均匀叠加态下傅里叶模式的解析界，增强素数与非素数的区分度。

Result: 新的解析界在中等实验偏差下提高了素数与非素数之间的分离度；噪声缓解方法有效抑制了实验噪声；在IBM量子处理器上成功实现了素数识别量子协议。

Conclusion: 该工作展示了在NISQ设备上运行数论相关量子协议的实际可行性，为未来量子计算在数论领域的应用迈出了重要一步。
...
TL;DR: 本文提出了强到弱自发对称性破缺(SW-SSB)的局域表述，通过局域单点保真度关联子来定义，相比之前的两点保真度关联子定义，在大系统检测和热力学极限下均具有优势，并在多个临界系统中验证了普适标度行为。


<details>
  <summary>Details</summary>
Motivation: 已有SW-SSB定义依赖两点保真度关联子，在大规模系统中检测困难，且热力学极限下密度矩阵本身定义不清，需要更实用的局域化定义。

Method: 提出局域单点保真度关联子替代两点保真度关联子，建立局域SW-SSB序参量；分析其在有限深度对称信道下的稳定性及长程条件互信息性质；在CFT基态、弹道/扩散自由费米子金属等临界系统中推导普适标度行为。

Result: 局域保真度序可在N规模系统中以poly(N)资源检测到O(log(N))体积尺度；局域SW-SSB序在热力学极限下良好定义；保留了SW-SSB的核心特征（稳定性、长程条件互信息）；在多种临界系统中得到普适标度律。

Conclusion: 局域SW-SSB框架在概念上与局域热化（ETH纯态）类似，为临界系统中的缺陷问题提供新视角，具有更广泛的实验可测性和理论普适性。
...
TL;DR: 本文证明量子交叉现象可通过局部测量（局部量子Fisher信息）鲁棒地表征，并发现量子肥胖不能普遍推广量子转向椭球体积作为交叉指示器，局部Bloch向量在某些情况下比椭球体积更敏感。


<details>
  <summary>Details</summary>
Motivation: 量子交叉现象在少体开放量子系统中至关重要，但传统识别方法依赖全局或模型依赖的指示器，缺乏鲁棒且普适的局部表征手段。

Method: 通过建立局部量子Fisher信息与交叉行为 onset 的直接联系，分析量子转向椭球体积与局部Bloch向量在不同交叉区间的敏感性差异。

Result: 局部量子Fisher信息可鲁棒表征交叉现象；量子肥胖不能普适推广椭球体积作为交叉指示器；椭球体积在某些区间不敏感，而局部Bloch向量编码了关键信号。

Conclusion: 揭示了局部与全局交叉指示器之间的几何差异，表明局部测量可作为更有效的交叉现象探测工具。
...
TL;DR: 提出了一种无需预先知道非酉传播子显式形式的量子算法，通过SVD分解将非酉传播子表示为酉算子之和，并在量子硬件上实现膨胀模拟，应用于光诱导电荷转移的量子主方程演示。


<details>
  <summary>Details</summary>
Motivation: 现有量子算法解决非自治线性ODE（非斜对称A(t)）时，需要预先知道非酉传播子的闭式表达并在经典计算机上进行计算和操作，这限制了算法的实用性。本文旨在消除这一先验要求。

Method: 结合SVD膨胀方案（将非酉传播子分解为酉算子之和）与在量子硬件上模拟SVD因子的动力学，从而在量子硬件上直接实现膨胀过程。

Result: 以描述溶剂化分子三元组中光诱导电荷转移的种群-时间无卷积量子主方程为演示实例，验证了算法的适用性及对噪声的敏感性。

Conclusion: 所提量子算法无需预先知道非酉传播子的显式形式，能够有效在量子硬件上处理非酉动力学问题，为开放量子系统等领域提供新的计算途径。
...
TL;DR: 线性储层计算的信息处理容量存在隐藏瓶颈：当特征线性演化且输出为线性读出时，任何固定延迟的容量已被预处理输入所限制；线性动力学只能重新分布特征，无法创造新的固定延迟表达能力。数值实验表明，单光子操作能超越这一高斯极限，成为量子储层计算的真资源。


<details>
  <summary>Details</summary>
Motivation: 揭示线性储层计算中信息处理容量的隐藏瓶颈，指出全局容量指标掩盖了单个延迟下的容量限制。

Method: 理论推导线性储层容量限制，推广到协方差基连续变量量子储层的高斯极限，并通过数值实验验证单光子操作超越该极限。

Result: 固定延迟下线性储层的容量受限于预处理输入；全局容量因不同延迟贡献累积而掩盖该限制；单光子操作超越高斯极限，成为量子储层计算的真资源。

Conclusion: 过量容量可作为黑箱连续变量系统中非高斯处理的运行见证，在最小假设下识别非高斯性。
...
TL;DR: 这篇教程系统介绍了现代量子纠错的核心概念，包括编码、综合征、稳定子、解码和容错性，并涵盖了拓扑码、子系统码、玻色子码、qudit码、动力学码和qLDPC码等主要码族及当前研究方向。


<details>
  <summary>Details</summary>
Motivation: 噪声是构建实用量子计算机的核心障碍，量子纠错为保护量子信息提供了框架。与经典纠错不同，量子纠错必须在不对量子态进行复制、直接测量或破坏其编码信息的前提下保护脆弱的量子态，这一挑战已成为量子信息科学最活跃的研究领域之一。

Method: 采用引导式教程的方式，系统性地从核心概念（编码、综合征、稳定子、解码、容错性）出发，逐步延伸到主要码族（拓扑码、子系统码、玻色子码、qudit码、动力学码、qLDPC码）和当前研究前沿。强调操作层面的理解，不仅解释主要对象是什么，还说明它们如何在码设计、错误诊断、解码和容错计算中被使用。

Result: 本文是一篇系统性的量子纠错教程，涵盖了从基础概念到前沿研究的完整知识框架。

Conclusion: 该教程旨在为量子纠错的初学者提供入门路径，同时也为研究人员提供关于概念、码族和工具的连贯参考。
...
TL;DR: 本文研究了非厄米拓扑系统中模块化结构对其独特物理性质的影响，包括非厄米趋肤效应、体-边界对应关系的破坏，以及增强的传感性能，特别是在谱拓扑相变附近和多参数估计场景中的临界增强灵敏度。


<details>
  <summary>Details</summary>
Motivation: 非厄米拓扑系统因其独特的物理性质和与开放量子系统的关联，近年来在理论和实验上受到广泛关注。本文旨在揭示模块化结构如何丰富这些系统的拓扑属性，并探索其在精密计量中的增强传感能力。

Method: 通过设计具有特定周期耦合值的模块化结构，分析非厄米趋肤效应和传统体-边界对应关系的改变，并研究系统在谱拓扑相变附近的传感性能，扩展到多参数估计场景。

Result: 模块化结构能够丰富非厄米系统的拓扑属性，如增强非厄米趋肤效应和破坏传统体-边界对应关系；系统展现出临界性增强的灵敏度，该增强在谱拓扑相变附近和多参数估计中均可实现。

Conclusion: 模块化结构是一种有效调控非厄米拓扑系统性质的手段，可显著提升系统的传感性能，为精密计量和量子传感提供新途径。
...
TL;DR: 本文研究了低纯度态在非纯度生成操作下是否可能产生纠缠。发现有些绝对可分态可以通过概率性协议产生纠缠，并提出了"完全绝对可分态"的概念，给出了完整分类。同时推导了仅依赖于最大最小特征值和最小局域维度的新的可分性充分条件。


<details>
  <summary>Details</summary>
Motivation: 低纯度态在实验上更容易制备，因此探究低纯度态是否可能产生纠缠具有实际意义。作者试图回答：一个态需要多高的纯度才能实现纠缠产生？

Method: 通过区分确定性单通道和概率性非纯度生成协议，引入"完全绝对可分态"概念，并利用谱域条件推导新的可分性判据。

Result: 发现某些绝对可分态可以通过概率性协议产生纠缠；给出了完全绝对可分态的完整分类；推导了仅依赖最大/最小特征值和最小局域维度且独立于已知谱可分性判据的新充分条件。

Conclusion: 低纯度态在概率性协议下仍可能产生纠缠，但完全绝对可分态在任何非零概率下都无法产生纠缠，这类态被完整刻画。
...
TL;DR: 本文提出一种混合高斯-指数外推方案，用于周期性量子电路的零噪声外推纠错，通过证明噪声放大因子弱收敛于对数正态分布，在中等至大深度电路上相比传统指数模型显著降低偏差。


<details>
  <summary>Details</summary>
Motivation: 零噪声外推的准确性高度依赖于噪声模型与实际误差缩放之间的保真度，现有方法对周期性量子电路（量子算法中普遍存在）的噪声建模精度不足，需要一种更精确且无需先验噪声表征的外推方案。

Method: 针对具有周期性结构的量子电路，在Pauli对角误差假设下，通过构建和分析Pauli算符转移的近似马尔可夫过程，证明了噪声放大因子弱收敛于对数正态分布，从而在标准指数模型中引入高斯方差修正，形成混合高斯-指数外推方案。

Result: 在Trotterized Ising动力学、随机电路和Grover搜索算法的Qiskit噪声模拟器上，对于中等至大深度电路，混合模型相比先前的外推变体实现了可测量的偏差降低。

Conclusion: 该混合高斯-指数外推方案无需先验噪声表征，可直接应用于任意周期性电路，能在近端量子硬件上有效抑制门误差，提升错误缓解效果。
...
TL;DR: 本文对比研究了置换矩阵表示（PMR）方法与其他主流量子算法在哈密顿量模拟中的资源消耗，发现PMR在时间无关和时间依赖两种场景下均具有资源需求和扩展性方面的互补优势，适合资源受限的量子硬件。


<details>
  <summary>Details</summary>
Motivation: 评估PMR方法在与QSP、qubitization（时间无关）及qHOP（时间依赖）等先进量子算法竞争中的实际资源效率，探索其在资源受限量子硬件上的实用潜力。

Method: 以Rydberg相互作用哈密顿量代表时间无关情形，以任意空间维度的Floquet驱动横场Ising模型代表时间依赖情形，分别对比PMR与QSP/qubitization以及PMR时间依赖扩展与qHOP的资源开销。

Result: PMR在两种情形下均展现出资源需求和扩展性方面的互补优势，尤其在特定系统参数下具有更优的扩展行为。

Conclusion: PMR可能在资源受限的量子硬件上提供实际效益，作为现有哈密顿量模拟方法的有益补充。
...
TL;DR: 本文提出了一种基于广义迹动力学(GTD)的条件性CSL型坍缩噪声关联子构造方案，通过引入辅助结构假设而非参数拟合得到谱形式，计算出的窄带谱位于哈勃尺度，与现有高频数据不冲突。


<details>
  <summary>Details</summary>
Motivation: 在GTD最小Grassmann代数框架下，纯费米子系数因幂零性缺乏普通符号、模和逆，导致$\mathrm{Tr}(q_F^\dagger\Gamma^\mu q_F)$到鬼模算子的约化受阻。作者希望通过引入辅助结构假设来克服这一障碍，构造候选的CSL型坍缩噪声关联子。

Method: 引入辅助正则费米子Fock空间替代原始幂零纯费米子系数（视为独立结构假设而非原始作用量的推论），采用标量双线性$J=\mathrm{Tr}(q_F^\dagger q_F)$作为浴算符，结合正模正则量子化和有效符号选择$\sigma=\pm1$，通过基本Wick收缩计算两点函数。

Result: 得到位于$|\omega|=2\omega_0$的Wightman谱线，振幅为$A_J=(\hbar/2m_R\omega_0 L_{\mathrm{aik}}^2)^2\cdot N\cdot D$。宇宙学识别$\omega_0\sim H_0$将谱线置于双倍哈勃尺度，该窄带谱位于当前CSL边界之外，不与现有高频数据冲突。

Conclusion: 所提出的构造不是来自最小GTD作用的无参数推导，而是基于一系列显式结构假设；在这些辅助结构下，谱形式和振幅通过计算而非现象学拟合得到，且框架不与现有高频数据存在张力。
...
TL;DR: 本文提出了混合态t-反相干性的公理化框架，基于对称量子比特嵌入区分了总反相干性和量子反相干性，前者在SU(2)协变通道下非递减，后者定义为相对于所选总测度的资源单调量且在纯态上与之重合。构建了基于约化态纯度、Hilbert-Schmidt距离和累积多极矩的总测度，以及基于凸包扩展的量子对应量，并给出了混合态示例、最大量子反相干态、粒子丢失鲁棒性以及纯度与反相干阶数之间的权衡分析。


<details>
  <summary>Details</summary>
Motivation: 混合态的反相干性可能来源于真正的量子关联或经典统计混合，现有研究缺乏统一的公理化区分框架，需要建立能够严格区分经典与量子贡献的理论体系。

Method: 采用对称量子比特嵌入构建公理化框架；定义总反相干性（SU(2)-协变通道下非递减）和量子反相干性（资源单调量）；基于约化态纯度、Hilbert-Schmidt距离、累积多极矩构建总测度；通过纯态泛函的凸包扩展构造量子对应量。

Result: 建立了混合态反相干性的严格分类体系；给出了显式混合态示例；识别了反相干子空间上的最大量子反相干态；分析了不同态在粒子丢失下的鲁棒性；刻画了纯度与最大可达反相干阶数之间的权衡关系。

Conclusion: 该框架成功区分了总反相干性与量子反相干性，揭示了经典统计混合与量子关联在反相干性中的不同角色，为方向无关计量学等应用提供了理论工具。
...
TL;DR: 本文提出一种混合ROI精细化框架，先通过经典或量子方法重建粗全局图像，再对选定ROI应用量子优化（QTR/QCSTR），在减少角度设置下实现高保真ROI重建。实验表明，SART+QTR/QCSTR组合在最大最复杂样本中取得最低ROI平均RMSE和MAE，证明量子优化更适合局部细化而非全局重建。


<details>
  <summary>Details</summary>
Motivation: 量子CT重建受限于图像表示所需的二进制变量数量，导致对大尺寸或结构复杂物体进行直接全图量子重建困难。

Method: 提出混合ROI精细化框架：先用QTR/QCSTR、FBP或SART重建粗全局图像，再通过残差投影-图像公式仅对选定ROI施加量子优化（QTR/QCSTR），从而减小有效QUBO规模。

Result: 在三个离散体模上的实验表明，QTR/QCSTR+QTR/QCSTR和SART+QTR/QCSTR在减少角度设置下均能实现中度大小样本的准确ROI重建；对于最大最复杂样本，粗全局估计质量至关重要，SART+QTR/QCSTR取得最低平均ROI RMSE和MAE。

Conclusion: 量子辅助CT重建的实际优势在于将量子优化保留用于局部细化，同时使用经典重建稳定全局背景。
...
TL;DR: 本文发展了双协方差模型(DCM)的相互作用推广，这是一个随机亚量子框架，通过关联微观涨落的粗粒化来涌现宏观量子动力学。利用多尺度伊藤积分和滑动窗口平均，从子系统希尔伯特空间上的局部随机微分方程推导出粗粒化双协方差算子的闭式演化方程，并在流体动力学极限下收敛到确定性的GKSL形式的宏观输运方程。


<details>
  <summary>Details</summary>
Motivation: 将双协方差模型从非相互作用系统推广到相互作用情形，建立统一的随机基础来同时描述开放和封闭量子动力学，并解释非可分离相互作用哈密顿量如何从严格局域的、状态依赖的随机反馈场中涌现。

Method: 采用多尺度伊藤积分和滑动窗口平均方法，显式引入快(微观涨落)和慢(宏观观测)两个分离的时间尺度，在流体动力学极限下（微观关联时间与平均窗口尺度之比趋于零）进行分析，得到粗粒化双协方差算子的闭式演化方程。

Result: 涌现的宏观动力学具有精确的GKSL形式：相干哈密顿演化来自确定性亚量子流，耗散通道来自二次噪声关联；非可分离相互作用哈密顿量可以从严格局域、状态依赖的随机反馈场中产生；在无涨落极限下，模型自然还原为标准冯·诺依曼方程。

Conclusion: 该框架为开放和封闭量子动力学提供了统一的随机基础，展示了如何从亚量子层次的微观随机过程通过粗粒化涌现出标准的量子主方程形式。
...
TL;DR: 本论文研究了BB84、SARG04和BBM92三种水下量子密钥分发（UQKD）协议在清澈、沿岸和浑浊三种非湍流海水中的性能，通过解析分析和蒙特卡洛模拟评估了量子比特误码率（QBER）和量子相关性随传播距离的变化，确定了各协议实现最优性能的条件。


<details>
  <summary>Details</summary>
Motivation: 探索水下环境中量子密钥分发的可行性和性能边界，为未来水下安全通信提供理论基础。

Method: 采用Kraus算子对BBM92协议的信道进行建模（结合衰减和退极化效应），推导QBER解析表达式；对三种协议在不同水质和系统参数下进行蒙特卡洛仿真验证。

Result: 蒙特卡洛仿真验证了所有协议的QBER解析表达式；确定了不同水质下各协议实现最优性能的条件。

Conclusion: 不同水下QKD协议的性能受水质、系统光学参数和信道物理效应的显著影响，需根据具体水下环境选择最优协议以实现安全高效的水下量子通信。
...
TL;DR: 本文应用样本基Krylov量子对角化（SKQD）方法研究受挫XXZ海森堡模型，在$J_1$-$J_2$正方晶格、Kagome晶格和一维链上对12至72自旋系统进行基态估计。通过引入ZZ变形（$\Delta=2$）、规范比特串压缩方案和多Krylov子空间改进，SKQD在24自旋以下达到亚百分偏差（如12位Kagome晶格相对误差0.002%，优于此前VQE的0.01%），并扩展至72自旋，虽误差升至19%-36%，但表明限制来自预算而非算法本身。


<details>
  <summary>Details</summary>
Motivation: 受挫晶格的量子自旋模拟对经典和量子算法都具有挑战性，尤其在量子自旋液体相的相关参数区域。现有方法难以兼顾精度和可扩展性，需要一种可扩展且与硬件兼容的量子模拟方法。

Method: 采用样本基Krylov量子对角化（SKQD）方法，并引入三项改进：(1) 识别$\Delta=2$的ZZ变形作为足够稀疏的哈密顿量；(2) 规范比特串压缩方案，在自旋翻转简并下保持构型恢复的有效性；(3) 使用多个Krylov子空间，在不增加量子资源的情况下改善基态覆盖。

Result: 在1D链和Kagome晶格上，SKQD在24自旋以下达到亚百分基态能量误差，12位Kagome晶格相对误差0.002%，超越此前最佳VQE结果（0.01%）。系统尺寸扩展至72自旋，误差从19%升至36%，呈渐进增长趋势。

Conclusion: SKQD为受挫海森堡模型的量子模拟建立了新基准，展示了一种可扩展、与硬件兼容的强关联自旋系统研究方法。误差随系统尺寸的渐进增长表明限制来自可用预算和电路深度，而非算法根本性约束。
...
TL;DR: 本文分析了由n量子比特泡利字符串生成的参数化酉门的Rademacher复杂度，给出了完整参数域下O(L^{3/2}/√M)和受限参数域下O(L/√M)的泛化界，并与经典线性模型进行了比较，发现可能存在统计复杂度优势。


<details>
  <summary>Details</summary>
Motivation: 虽然已有一些量子机器学习模型的泛化界研究，但针对泡利字符串生成的参数化酉门的明确Rademacher复杂度界限仍然不够清晰透明，本文旨在填补这一空白。

Method: 理论推导了参数化酉门（由n量子比特泡利字符串生成）的Rademacher复杂度上界，考虑了完整参数域和受限参数域两种情况；并通过数值实验定性验证了理论缩放规律。

Result: 完整参数域下Rademacher复杂度为O(L^{3/2}/√M)，受限参数域下为O(L/√M)；与经典线性模型比较发现，当输入范数和参数范数均随参数数量缩放时，量子模型可能具有统计复杂度优势。

Conclusion: 建立了泡利字符串生成参数化酉门的Rademacher复杂度显式界限，揭示了与经典模型相比的潜在优势，并通过数值实验提供了定性支持。
...
### [38] [Bell's theorem: why probability factorisation fails](https://arxiv.org/abs/2605.29589)
*Geoff Beck*

Main category: quant-ph

TL;DR: 本文通过分析单自旋序列实验与双自旋纠缠实验的反事实等价性，论证贝尔型关联本质上只是序列实验的不同实现方式，从而将表观非局域性解释为序列自旋测量中状态改变导致的间接测量效应。


<details>
  <summary>Details</summary>
Motivation: 尽管贝尔不等式 violation 的实验验证取得了里程碑式成功，但它迫使我们接受一种缺乏严格相对论局域性的宇宙，或者通过多世界、超决定论等潜在漏洞逃避。作者希望寻求一种替代解释，避免上述困境。

Method: 研究一个自旋序列实验（sequential, single-spin experiment），其中自关联（auto-correlations）与贝尔场景中双自旋纠缠关联（entangled correlations）相匹配。利用两者之间的反事实等价性（counterfactual equivalence）论证贝尔型关联本质上是序列实验的不同实现。

Result: 建立了单自旋序列实验与双自旋贝尔实验之间的反事实等价性；证明贝尔型关联可被理解为序列实验的另一种实现方式。

Conclusion: 表观上的非局域性实际上是序列自旋测量中测量改变状态，导致间接测得了不具有预定义值的量的结果。这一观点与原初EPR论证的反事实基础一致。
...
### [39] [Error-corrected phase estimation averaged over variable grids on a trapped-ion quantum computer: hyperacuity spectra of a CO molecule adsorbed onto $χ$-Fe$_5$C$_2$](https://arxiv.org/abs/2605.29674)
*Taichi Kosugi,Hirofumi Nishi,Keito Kasebayashi,Hiroki Takahashi,Yu-ichiro Matsushita*

Main category: quant-ph

TL;DR: 本文提出QAVG（变网格平均量子相位估计）方法，通过结合低分辨率QPE与多原点平移及连续参数化，在含噪环境下精确重建光谱，并在Quantinuum H2-2上以CO/χ-Fe₅C₂模型系统验证了其超越标称分辨率的性能。


<details>
  <summary>Details</summary>
Motivation: 量子相位估计（QPE）是提取多电子系统激发谱的关键技术，但当前硬件受限于低网格分辨率和环境噪声，限制了其实用性。

Method: 提出QAVG方法，采用游标卡尺式策略：将低分辨率QPE与多个原点平移相结合，并引入物理驱动的连续参数化，通过平移到不同网格上的直方图平均来精确重构光谱。

Result: 在Quantinuum H2-2上对CO分子吸附在χ-Fe₅C₂表面的模型系统进行实验，使用物理QPE电路和Steane码编码的逻辑QPE电路（带离线比特翻转纠正），结果显示QAVG重构的光谱偏差远小于标称QPE分辨率，且平移网格上的成本景观平均有效抑制了由谱泄漏导致的局部极小值。

Conclusion: QAVG为早期容错量子计算机时代的相关谱量子模拟提供了一条稳健的路径。
...
### [40] [Channel-agnostic finite-temperature phase estimation averaged over variable grids: reconstruction of Green's function for dynamical mean-field theory](https://arxiv.org/abs/2605.29681)
*Taichi Kosugi,Hirofumi Nishi,Keito Kasebayashi,Hiroki Takahashi,Yu-ichiro Matsushita*

Main category: quant-ph

TL;DR: 提出了一种用于量子计算机上处理关联电子系统的量子-经典混合动力学平均场理论（DMFT）方案。量子部分采用改进的量子相位估计（QPE）电路，可在有限温度下提取单粒子格林函数的谱振幅和激发能量；经典部分提出QPE可变网格平均（QAVG）方法，通过优化试探参数和建模概率分布来重构格林函数。在SrVO₃上的数值模拟验证了该方案的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有量子计算方案在处理关联电子系统的动力学平均场理论时，面临有限温度下单粒子格林函数的谱信息提取困难，且无法有效区分不同激发通道的问题。传统QPE方法难以直接应用于有限温度GF的计算，需要发展新的量子-经典混合方案来解决这一瓶颈。

Method: （1）量子部分：设计改进的QPE电路，使其能在有限温度下提取单粒子格林函数的谱振幅和激发能量，且无需在每次测量时先验知道激发通道；（2）经典部分：提出QAVG方法，通过对不同QPE电路设置下的概率分布进行建模，并优化试探参数，从而合理估计格林函数；（3）将两者结合为QAVG-DMFT混合方案，并在SrVO₃材料上进行数值模拟验证。

Result: 在SrVO₃的数值模拟中，QAVG-DMFT方案成功重构了单粒子格林函数，验证了该混合方案在处理关联电子系统时的有效性和可行性。

Conclusion: QAVG-DMFT方案为在量子计算机上处理关联电子系统提供了一条可行的混合计算路径，量子部分解决了有限温度下GF的谱信息提取问题，经典部分通过QAVG方法实现了GF的合理重构。该方法在真实材料（SrVO₃）上的成功验证表明其具有良好的应用前景。
...
### [41] [Finite-key feasibility of geostationary quantum key distribution](https://arxiv.org/abs/2605.29706)
*Vaisakh Mannalath,Víctor Zapatero,Marcos Curty*

Main category: quant-ph

TL;DR: 本文对基于地球静止轨道(GEO)卫星的量子密钥分发(QKD)下行链路进行了全面的端到端可行性研究，采用诱骗态BB84协议、可变长度有限密钥安全性和严格统计界限，结合真实信道模型和欧洲历史云层数据，评估了多种波长、接收器架构及环境条件下的年度密钥生成性能，为未来GEO-QKD任务提供了实用设计指南。


<details>
  <summary>Details</summary>
Motivation: GEO卫星QKD能够实现连续、大陆规模的安全通信，但面临极端信道损耗和显著背景噪声（尤其白天运行）的严峻挑战，此前缺乏端到端的系统可行性评估。

Method: 采用诱骗态BB84协议，结合可变长度有限密钥安全性与紧致统计界限，对多种接收器架构（含可见光Fraunhofer吸收极小波长和电信波段）进行建模，利用物理真实的信道模型（捕获主要损耗和噪声机制）以及欧洲历史云层数据预测年度密钥产量。

Result: 系统探索了高维参数空间，识别了决定可行性的关键权衡和性能瓶颈，建立了实用运行阈值，预测了欧洲大陆多环境（乡村、城市、沿海）下的年度密钥产量。

Conclusion: 研究结果为未来GEO-QKD任务提供了可操作的设计准则和实用运行阈值，证实了GEO卫星实现连续、大陆尺度安全通信的可行路径。
...
### [42] [Discrete and Continuous Wigner Functions in Open Quantum Systems: Non-Markovian and Thermodynamic Effects](https://arxiv.org/abs/2605.29717)
*Jai Lalita*

Main category: quant-ph

TL;DR: 本论文研究了有限维量子系统中非经典资源的识别、表征和保护，利用离散Wigner函数框架引入负量子态，分析其在各种噪声通道下的演化，并通过弱测量和量子测量反转策略增强其性能。在IBM超导量子硬件上成功制备了这些态，验证了其在纠缠、CHSH不等式违背和Fisher信息方面优于Bell态的表现。


<details>
  <summary>Details</summary>
Motivation: 在现实噪声环境下，非经典量子资源（如负性态）的识别、表征和保护是量子信息处理的关键挑战，尤其是在非马尔可夫噪声普遍存在的场景中，需要寻找比传统Bell态更具鲁棒性的替代资源。

Method: 基于离散Wigner函数框架定义负量子态；分析其在单分和非幺正通道下的演化（Wigner负性、mana、相干性、保真度）；采用弱测量和量子测量反转进行保护；在IBM超导硬件上设计并实现量子电路，通过量子态层析验证。

Result: 弱测量和量子测量反转能增强量子关联、降低保真度偏差、提升隐形传态性能；相空间点算子导出的负性态在非马尔可夫噪声下比Bell态具有更强的纠缠鲁棒性；在IBM ibm-Brisbane设备上成功制备这些态并验证高保真度；这些负性态在CHSH不等式违背和Fisher信息方面优于Bell态。

Conclusion: 负量子态有望在非马尔可夫错误普遍的场景中替代传统Bell态，作为更优的量子资源使用。
...
### [43] [Treewidth-Aware Gate Cut Selection for Reducing Transpilation Overhead on Superconducting Quantum Devices](https://arxiv.org/abs/2605.29723)
*Hana Ebi,Shin Nishio,Takahiko Satoh*

Main category: quant-ph

TL;DR: 提出TW2S，一种基于电路交互图的两阶段门切割选择方法，通过在Min-fill消去迹中评分边对树宽上限的贡献，并利用边介数中心度与度惩罚识别路由瓶颈。在IBM FakeSherbrooke后端上的实验表明，当交互图存在可识别的稀疏切割时，TW2S显著优于随机切割选择，并在J1-J2横场Ising模型噪声仿真中实现ΔECR=47（n=8），相比随机选择的约9有大幅提升，且在强信号区降低了估计误差。


<details>
  <summary>Details</summary>
Motivation: 超导量子设备中稀疏的量子比特连接导致长程双量子比特门在转译时需插入大量SWAP门，增加硬件成本和执行错误。门切割（通过准概率分解移除选定门）可减少路由开销，但其采样成本使切割位置的选择至关重要。现有方法缺乏一种无需后端特定转译的图结构切割选择方案。

Method: TW2S采用两阶段图分析方法：第一阶段通过Min-fill消去迹分析边对树宽上限的贡献并评分；第二阶段利用边介数中心度和度惩罚对候选边排序，识别路由瓶颈。该方法在电路交互图上操作，无需在选定时进行后端特定转译。

Result: 在网格图、Watts-Strogatz、barbell和随机块模型基准上转译到IBM FakeSherbrooke后端，TW2S在交互图存在可识别稀疏切割时持续优于随机选择。在J1-J2横场Ising模型噪声仿真中，n=8时TW2S实现ΔECR=47（随机选择约9），在强信号区且增加采样预算时估计误差低于未切割基线。推导了均方误差盈亏条件：共享总预算下QPD仅在ECR减少足够大且信号强度充分时有利；扩展子电路预算下信号强度要求大幅放宽。

Conclusion: 图结构切割选择可作为一种实用的编译器手段，将电路切割转化为有针对性的路由减少策略。切割优势取决于适度的社区结构和可访问的社区间边，而非绝对图密度。
...
### [44] [Bound-state-protected phase metrology for a quantum emitter in a Su-Schrieffer-Heeger bath](https://arxiv.org/abs/2605.29724)
*Sofia Evangelou*

Main category: quant-ph

TL;DR: 本文研究了单个量子发射器与Su-Schrieffer-Heeger (SSH) 玻色子浴耦合时的局域相位估计问题。二聚化打开了中心能隙，产生了能隙内发射器-浴束缚态，从而抑制了发射器相干性的完全弛豫。通过Dyson方程分析得到了局域浴格林函数、能隙内束缚态条件以及控制保留相位信息的发射器权重。相位量子Fisher信息将能隙形成、失谐和发射器-浴耦合与瞬态后的计量响应联系起来。当共振时，强耦合增强瞬态杂化但降低保留信号；远离共振时，相位信息保护随发射器调谐至带边及以外而减弱。均匀链对照显示能隙闭合时保留信号消失。由于采用体态局域耦合几何，响应对于二聚化符号不敏感，因此该协议探测的是能隙物理和束缚态支撑，而非SSH缠绕扇区。


<details>
  <summary>Details</summary>
Motivation: 探究量子发射器与拓扑非平凡环境（SSH浴）耦合时的局域相位估计能力，理解能隙内束缚态如何保护量子相干性并影响计量灵敏度，揭示非马尔可夫浴对量子传感性能的调控机制。

Method: 采用微观晶格模型，通过Dyson方程分析求解局域浴格林函数和能隙内束缚态条件；利用量子Fisher信息量化相位估计的计量响应；分析发射器残余权重、瞬态杂化、延迟时间平均等指标，并与均匀链对照比较。

Result: 二聚化打开能隙并产生束缚态，抑制发射器完全弛豫；强耦合增强瞬态杂化但降低发射器在束缚态中的权重，从而减少保留信号；相位信息保护在发射器调谐至带边时减弱；均匀链对照证实能隙闭合时信号消失；体态耦合下响应不依赖二聚化符号，仅反映能隙与束缚态物理。

Conclusion: 该协议通过SSH浴的能隙内束缚态实现对发射器相位信息的保护，但其响应仅反映能隙物理而非拓扑缠绕扇区，因此在体态局域耦合几何中无法用于探测拓扑相位。
...
### [45] [Non-Perturbative Closed Form for the Typical Bipartite Mutual Information of Haar-Random States](https://arxiv.org/abs/2605.29725)
*Zhi-Wei Wang,Pei-Wen Li,Samuel L. Braunstein*

Main category: quant-ph

TL;DR: 本文推导了Haar随机纯态的平均二分量子互信息⟨I(A:B)⟩的精确非微扰闭式，其表达式为(d_A²-1)(d_B²-1)乘以一个由Bose-Einstein核收敛积分定义的函数𝒢。该积分表示揭示了渐近展开的Borel可和性，并通过尺度反演对称性给出了上界。推导过程将Page熵分解为对角Dirichlet贡献和Schur排序本征值修正，清晰分离了经典与量子关联。


<details>
  <summary>Details</summary>
Motivation: 量子互信息是度量量子系统中经典与量子关联的关键量，但此前对Haar随机纯态的⟨I(A:B)⟩仅有通过digamma函数表达的Page公式，缺乏简洁的非微扰闭式。本文旨在找到该量的精确闭合形式，并揭示其与李代数维数、Borel可和性及关联分离的深层结构联系。

Method: 作者通过将Page熵精确分解为Dirichlet边界贡献与Schur本征值修正，将互信息重新组装为(d_A²-1)(d_B²-1)因子与一个Bose-Einstein核积分𝒢的乘积。利用尺度反演对称性证明上界，并通过Borel求和处理渐近展开的发散性。

Result: 得到了⟨I(A:B)⟩的精确非微扰闭式：⟨I(A:B)⟩ = (d_A²-1)(d_B²-1)𝒢(d_A,d_B,d_E)，其中𝒢由∫₀^∞ t e^{-t}(1-e^{-t})^{-1}(1-e^{-d_A t})^{-1}(1-e^{-d_B t})^{-1} e^{-d_E t} dt形式的积分给出。渐近展开产生涉及ζ(1-2k)的Bernoulli因子化级数，该级数发散但被积分精确Borel求和。上界⟨I⟩ < (d_A²-1)(d_B²-1)/(2N)由核的尺度反演对称性直接得到。

Conclusion: 该工作建立了量子互信息与李代数维数(d_A²-1)(d_B²-1)的精确联系，揭示了其渐近展开的Borel可和性，并通过熵分解方法清晰区分了经典与量子关联贡献，为理解随机量子态的关联结构提供了新视角。
...
### [46] [Exact Geometric Typicality and Bipartite Entanglement from the Projected Central Limit Theorem on Hyperspheres](https://arxiv.org/abs/2605.29732)
*Zhi-Wei Wang,Pei-Wen Li,Samuel L. Braunstein*

Main category: quant-ph

TL;DR: 本文从超球面上的精确投影中心极限定理出发，重新推导了子系统占据概率的Beta分布和Lubkin纯度公式，并给出了Haar随机纯态下二分量子互信息的完整渐近展开，证明其具有伯努利分解形式，并建立了非微扰闭式解。


<details>
  <summary>Details</summary>
Motivation: 现有本征态热化假说和典型性处理中使用了高斯近似，但该近似在有限尺寸下存在尾部"platykurtic"抑制偏差。作者希望精确量化这种偏差，并推导Haar随机纯态下二分量子互信息的精确渐近行为。

Method: 从超球面精确投影中心极限定理出发，利用初等高维球谐矩重新推导Beta分布和Lubkin纯度公式；通过Page公式的精确代数重组，利用Schur优超定理和广义Bloch分解的Cartan结构，将互信息分解为量子相干贡献和经典概率贡献。

Result: 证明了互信息渐近展开中每个k≥1阶携带对称因子(d_A^{2k}-1)(d_B^{2k}-1)，所有奇次校正恒为零；主导有限尺寸修正项分离为su(d_A)⊗su(d_B)双向量子相干贡献(d_A^2-1)(d_B^2-1)/(2N)和经典Cartan贡献(d_A-1)(d_B-1)/(2N)。给出了非微扰闭式：⟨I(A:B)⟩ = (d_A^2-1)(d_B^2-1)𝒢(d_A,d_B,d_E)，其中𝒢为显式Bose-Einstein积分。

Conclusion: 建立了Haar随机纯态二分量子互信息的精确解析框架，揭示了有限尺寸校正的伯努利级数结构和因子化形式，为理解量子纠缠的典型行为提供了非微扰理论基础。
...
### [47] [Quantum algorithms for density functional theory with minimal readout](https://arxiv.org/abs/2605.29774)
...
TL;DR: 本文提出了一种用于Kohn-Sham DFT的量子算法，通过量子比特高效的波函数编码方案同时计算所有占据轨道，并利用Harris泛函避免电子密度读出，从而在总能量评估中实现潜在的指数级加速。同时提出了一种利用波函数多副本实现自洽DFT计算的方法。


<details>
  <summary>Details</summary>
Motivation: 量子计算机在电子结构计算中显示出潜力，但其加速密度泛函理论计算的能力尚不明确，本文旨在探索量子算法在DFT中的加速可能性。

Method: (1) 提出量子比特高效的波函数编码方案，用于Kohn-Sham DFT中的波函数表示；(2) 设计量子算法同时计算所有占据轨道；(3) 利用Harris泛函避免电子密度读出，实现总能量的潜在指数级加速；(4) 提出基于波函数多副本的自洽DFT计算方法，同样规避密度读出。

Result: 通过多个数值示例验证了所提算法的适用性，并与现有方法进行了效率比较。

Conclusion: 所提出的量子算法能够有效加速DFT计算，特别是在Harris泛函框架下可实现潜在指数级加速，为量子计算在材料科学和化学中的应用提供了新途径。
...
### [48] [Incompleteness is necessary for activation of nonlocality without entanglement](https://arxiv.org/abs/2605.29775)
*Atanu Bhunia,Saronath Halder,Preeti Parashar,Ritabrata Sengupta*

Main category: quant-ph

TL;DR: 本文研究了正交乘积态的“无纠缠量子非局域性”现象，证明了任何完全正交乘积基在保持正交性的局域投影测量和经典通信下不会激活隐藏非局域性，并形式化了“强局域集”概念，为量子态区分中的局域-非局域转变提供了结构性理解。


<details>
  <summary>Details</summary>
Motivation: 理解正交乘积态在局域操作和经典通信下的可区分性边界，特别是探索“激活隐藏非局域性”现象是否普遍存在，以及刻画局域可区分性不可逆丧失的条件。

Method: 通过理论证明建立：任何完全正交乘积基若初始局域可区分，则在所有保持正交性的局域投影测量和经典通信下仍保持可区分性；引入并形式化“强局域集”概念，研究多分体系统中局域可激活性的边界。

Result: 证明了完全正交乘积基的不可激活性定理，排除了通过保持正交性的局域投影测量激活隐藏非局域性的可能性；定义了“强局域集”并揭示了其在不同二分划下的不可激活特性。

Conclusion: 该工作为量子态区分中局域性到非局域性的转变提供了严格的结构性理解，有助于刻画LOCC可区分性及其不可逆丧失之间的边界。
...
### [49] [Resolving the phase space](https://arxiv.org/abs/2605.29784)
*Zdenek Hradil,Jaroslav Rehacek*

Main category: quant-ph

TL;DR: 本文建立了基于分辨率的量子层析框架，指出层析的有效分辨率由测量Gram矩阵定义的采样算子决定，该算子类似于成像中的传递函数，可用于区分真实量子特征与重构伪影。


<details>
  <summary>Details</summary>
Motivation: 量子层析能够重建测量本身未必分辨的精细相空间结构，但缺乏明确标准来区分真实特征与重构伪影，需要建立分辨率理论框架。

Method: 通过Gram矩阵定义采样算子，类比成像中的传递函数，在Gram本征基下进行重构实现测量自适应压缩，并从有限帧理论角度建立与帧算子的联系。

Result: 证明了层析有效分辨率由采样算子决定，该算子定义了实验可访问的自由度和重构带宽；Gram本征基重构可高效压缩层析问题。

Conclusion: 建立了适用于当代探测高结构非经典态实验的基于分辨率的量子层析框架，提供了区分真实特征与伪影的操作性判据。
...
### [50] [Chain rules for conditional entropies in quantum cryptography: limitations and improvements](https://arxiv.org/abs/2605.29787)
*Lewis Wooltorton,Peter Brown,Omar Fawzi*

Main category: quant-ph

TL;DR: 本文研究了量子密码学中条件熵的链式法则，发现设备无关（DI）场景下无法实现类似可信设备场景中的紧致i.i.d.约化，但提出了一个新的链式法则，并基于此给出了Rényi熵累积定理的略微紧化版本，同时构建了统一现有链式法则的框架。


<details>
  <summary>Details</summary>
Motivation: 在量子密码学的多轮协议中，安全证明依赖于条件熵的估计，而最一般攻击（非i.i.d.）使得这一估计极具挑战。链式法则通过将结构化非i.i.d.过程的熵分解为各轮贡献之和来解决此问题。在可信设备场景中，链式法则已实现有限轮数下的紧致i.i.d.约化，但能否在设备无关（DI）场景中恢复类似结果尚不清楚。本文旨在探索DI场景下链式法则的局限性并寻求改进。

Method: 首先证明Dupuis等人链式法则的自然紧化在DI场景下不成立，揭示当前DI安全证明方法的局限。随后证明一个新的链式法则，并基于Arqand等人的框架，利用该法则给出Rényi熵累积定理在特定语境下的略微紧化版本。最后构建一个自洽的框架来统一现有链式法则并比较其应用。

Result: 证明了DI场景下链式法则存在固有的紧致性限制，即Dupuis等人链式法则的自然紧化不可能成立。同时提出了一个新的链式法则，在DI场景下实现了中间程度的改进，并据此得到了Rényi EAT的略微紧化版本。此外，建立了统一现有链式法则的框架。

Conclusion: 本文揭示了设备无关量子密码学中当前安全证明方法的一个根本性限制——无法实现类似可信设备场景的紧致i.i.d.约化，但通过新的链式法则和统一框架为后续改进提供了方向和工具。
...
### [51] [Adaptive Stabilizer State Fidelity Certification](https://arxiv.org/abs/2605.29820)
*Kun Wang*

Main category: quant-ph

TL;DR: 本文针对稳定子态保真度认证中的规范依赖问题，提出了自适应扩展方法，通过优化上下界和规范选择策略，实现了完整的认证区间报告，并证明了单调收紧、精确恢复等性质，数值验证了自适应方法在特定症候群分布下的加速收敛。


<details>
  <summary>Details</summary>
Motivation: 现有稳定子态保真度认证方法在固定规范下只能给出最坏情况下的下界，且规范依赖会导致保真度存在较大模糊性，需要开发能报告完整认证区间的方法。

Method: 推导了单规范下的互补最优最坏情况上界；将规范选择建模为自适应设计问题，每轮通过求解精确端点线性规划并采用见证消除策略选择新规范；证明了单调收紧、完全覆盖后精确恢复以及最坏情况下全覆盖必要性等理论性质。

Result: 在特定结构化症候群分布下，自适应方法可超越指数基准；数值实验证实了更快的收敛速度。

Conclusion: 提出的自适应扩展方法能有效解决稳定子态保真度认证中的规范模糊性问题，在理论上保证了单调收敛和精确恢复，数值上展示了优于传统方法的性能。
...
### [52] [Indistinguishability of photonic qubits emitted from trapped $^{40}$Ca$^+$ ions via pulsed excitation](https://arxiv.org/abs/2605.29825)
*Pascal Baumgart,Max Bergerhoff,Jonas Meiers,Stephan Kucera,Jürgen Eschner*

Main category: quant-ph

TL;DR: 本文研究了利用纳秒级激发脉冲从两个捕获的$^{40}$Ca$^+$离子产生的拉曼光子的不可区分性，阐明了自发散射回初始态如何影响Hong-Ou-Mandel干涉，并确定了反向衰变的平均次数作为可测量的单发射体量，与来自两个相同发射体的光子可实现干涉可见度相关。


<details>
  <summary>Details</summary>
Motivation: 探究从两个独立 trapped 离子产生的拉曼光子的不可区分性，以及自发散射回初始态对Hong-Ou-Mandel干涉的影响。

Method: 使用纳秒级激发脉冲从两个捕获的$^{40}$Ca$^+$离子产生拉曼光子，通过Hong-Ou-Mandel干涉实验测量光子不可区分性，并分析反向衰变平均次数与干涉可见度的关系。

Result: 反向衰变的平均次数可以作为可测量的单发射体量，与两个相同发射体产生的光子干涉可见度相关。

Conclusion: 阐明了自发散射回初始态对Hong-Ou-Mandel干涉的影响，并提出了反向衰变平均次数作为预测光子干涉可见度的关键参数。
...
### [53] [Gate Parameter Lee-Yang Zeros and Dynamical Phases in Quantum Circuits](https://arxiv.org/abs/2605.29838)
*Chang Liu,Yu Wu,Yunfeng Jiang,Yang Zhang*

Main category: quant-ph

TL;DR: 本文提出将门参数李-杨零点（Lee-Yang zeros）作为有限量子电路中动力学相的探针，通过砖墙模型展示了Loschmidt振幅中零点的凝聚行为，并证明其可作为动力学相变的有限比特诊断工具。


<details>
  <summary>Details</summary>
Motivation: 现有的动力学相变诊断方法通常依赖热力学极限或精确可积性，缺乏适用于有限量子电路且不依赖可积性的通用探针。

Method: 采用砖墙模型（brickwork model），通过重复应用Floquet算符生成时间演化；将Loschmidt振幅表示为门参数的有理函数；在固定系统尺寸和大电路深度下，研究一个复化门参数中零点的凝聚曲线；利用Beraha-Kahane-Weiss定理描述普适分量，结合态依赖贡献进行完整刻画。

Result: 零点凝聚曲线由两部分组成：基于等模Floquet本征值的普适分量（由Beraha-Kahane-Weiss定理描述）和依赖于初态与Floquet算符本征态重叠的态依赖贡献。当参数变化时，零点集合发生重组，作为有限比特系统中动力学相变的诊断信号。

Conclusion: 该机制不依赖可积性，仅凭谱竞争和局域酉性即可实现零点的凝聚，为有限量子电路中动力学相变的探测提供了通用框架。
...
TL;DR: 本文通过实验演示了一种基于窄带激光源的双向隐蔽通信系统，并提出了无需精确模式匹配即可实现量子光源宽带增益的关联接收机方案。


<details>
  <summary>Details</summary>
Motivation: 宽带经典和量子光源虽理论上可提供高隐蔽吞吐量，但其模式匹配和相位同步要求过高，缺乏实用性，因此需要寻求更实用的窄带方案。

Method: 采用窄带激光源构建双向隐蔽通信实验系统，假设对手具备量子能力；提出基于相关器的接收机结构，以规避精确模式匹配的需求。

Result: 成功实验演示了基于窄带激光源的双向隐蔽通信概念验证系统；所提相关器接收机能够获得量子光源的宽带增益，无需精确模式匹配。

Conclusion: 窄带激光源结合相关器接收机可实现实用的双向隐蔽通信，克服了宽带光源在模式匹配和相位同步方面的实际障碍。
...
TL;DR: 系统综述81篇近期QEM论文后发现，仅25%使用推断统计方法，42%仅描述性报告不确定性。通过ZNE案例研究识别出两类人为伪影：参数敏感性（132种配置下结论可从显著改善变为显著退化）和漂移诱导的有效性幻觉（72小时纵向实验中时间漂移可使效应量增大三倍以上）。提出最低报告标准。


<details>
  <summary>Details</summary>
Motivation: 尽管QEM被视为从NISQ到FTQC的关键桥梁，但其有效性评估的实证研究方法论尚未受到充分审视，现有结论的有效性存疑。

Method: 采用八准则框架系统评审81篇QEM论文，并通过ZNE案例进行参数敏感性扫描（132种配置）和72小时纵向硬件实验。

Result: 仅25%论文使用推断统计方法；参数选择可导致结论反转；时间漂移可使效应量增大三倍以上并减少有效独立观测数。

Conclusion: 当前QEM评估实践可能使缓解性能表现得更稳健，需建立包括参数文档、稳健性检验、纵向漂移评估和推断统计测试的最低报告标准。
...
TL;DR: 本文研究了非球形中性粒子在自由空间中旋转时与电磁量子真空的相互作用。当旋转轴垂直于粒子对称轴时，散射场产生频率边带，诱导参量性动态卡西米尔光子对发射。在最大尖端速度约束下，近球形几何结构可最大化发射率，且在极化激元共振附近进一步增强。然而，即使优化后，实际材料参数下的发射上限仍然极小，为单纳米粒子的自由空间旋转动态卡西米尔发射设定了严格的定量限制。


<details>
  <summary>Details</summary>
Motivation: 探索非球形中性粒子在自由空间旋转时与量子真空相互作用产生动态卡西米尔光子对的物理机制，并定量评估其可行性边界。

Method: 理论分析非球形粒子旋转时散射场的频率边带效应，在最大尖端速度约束下优化几何参数（近球形）和材料参数（极化激元共振），计算动态卡西米尔光子对的发射率。

Result: 即使在最优条件下（近球形几何、极化激元共振增强），单纳米粒子的自由空间旋转动态卡西米尔发射率仍然极小。

Conclusion: 自由空间旋转的单纳米粒子无法产生可观测的动态卡西米尔效应，这一结果为相关理论和实验研究提供了严格的定量限制。
...
TL;DR: 本文评估了FALQON参数迁移方法在Max-Cut问题上的表现，发现迁移成功与否取决于接收图而非捐赠图，密集接收图可获得高近似比，且小规模图（8节点）即可为更大目标提供稳健参数，显著降低测量开销。


<details>
  <summary>Details</summary>
Motivation: FALQON算法需要大量反馈循环测量，计算开销高昂。作者希望探索参数迁移方法，利用小规模图计算得到的参数直接应用于大规模图，以减少测量开销。

Method: 将来自小规模捐赠图（8、10、12节点）的FALQON参数序列迁移到14节点接收图上，在3-正则图和Erdős-Rényi图两种图族中测试迁移效果，分析捐赠图与接收图结构对迁移成功率的影响。

Result: 迁移成功由接收图结构主导而非捐赠图；密集接收图可获得高近似比；跨图族稀疏场景仍具挑战性；捐赠图规模对性能影响很小，8节点捐赠图即可达到与大尺寸图相当的效果。

Conclusion: 廉价的小规模图可以为核心参数提供稳健的迁移来源，显著降低FALQON反馈循环的测量开销，为实现可扩展的量子优化提供了实用路径。
...
TL;DR: 本文针对k-局域哈密顿量问题，证明了在Pauli设定下，NPA型下界层次和上界层次在谱最小值上的显式有限级收敛速率，误差上界为C(k)ξ^{n,4}_{d+1}/n，其中ξ^{n,4}_{d+1}是Krawtchouk多项式的最小根。该结果首次为非交换半定松弛提供了定量有限级精度保证。


<details>
  <summary>Details</summary>
Motivation: k-局域哈密顿量问题是量子多体系统和哈密顿量复杂性的核心模型。半定规划和非交换平方和层次提供了基态能量的系统化认证，但现有的有限收敛结果无法给出计算中可访问的低层级精度的定量保证。

Method: 构建Pauli代数的几乎再生核，将其谱与Krawtchouk多项式关联，从而建立非交换版本的内核收敛分析框架。对于仅含偶重Pauli项的k-局域哈密顿量，直接证明误差上界；对于一般k-局域哈密顿量，通过添加一个辅助量子比特保持谱结构归约到偶重情形。

Result: 证明了NPA型下界层次和上界层次在谱最小值上的误差至多为C(k)ξ^{n,4}_{d+1}/n，其中C(k)与量子比特数n和层次级数d无关。这是非交换半定松弛在Pauli哈密顿量上的首个定量有限级精度保证。

Conclusion: 该工作填补了非交换多项式优化中有限级收敛定量分析的空白，为量子多体系统的可计算认证提供了理论基础，并建立了与交换多项式优化中基于核的收敛分析之间的非交换类比。
...
TL;DR: 本文改进了量子行走工具箱中的电场流采样（elfs）工具，证明了存在零误差的 transducer 实现 elfs，并建立了关于两个子空间交集的零误差 transducer，由此获得了有效电阻估计、跨程序见证者大小估计以及随机游走到达分布采样的最优误差缩放量子行走算法，并在扩展图上的半监督学习中实现了最高二次的量子加速。


<details>
  <summary>Details</summary>
Motivation: 电场流采样（elfs）是量子行走工具箱中的新工具，可用于解决图上的搜索、采样和优化问题。本文旨在改进这一工具，消除其误差，并基于此获得更优的量子算法。

Method: 1. 证明了存在零误差 transducer 用于实现 elfs；2. 建立了更广泛的关于两个子空间交集的零误差 transducer，即误差自由的 transducer 版本的有效间隙引理；3. 基于此结果，通过组合多个 elfs 得到改进的量子行走算法。

Result: 1. 获得了有效电阻估计和跨程序见证者大小估计的量子行走算法，具有最优误差缩放；2. 获得了随机游走到达分布采样的量子算法；3. 在扩展图上的半监督学习中实现了最高二次的量子加速。

Conclusion: 零误差 transducer 的建立使得 elfs 及相关量子行走算法在误差方面达到最优，并带来了实际应用（半监督学习）的量子加速，展示了 elfs 作为量子行走基础工具的潜力。
...
TL;DR: 本文提出一种几何机制，解释非幺正噪声（如振幅阻尼信道）结合通用量子门集如何加速量子态向哈尔分布趋近：噪声诱导纯态流形上的有效体积膨胀，通过最小1量子比特模型推导局部面积扩张因子与全局扩张阈值，并证明在适当参数区域噪声可作为量子算法的资源。


<details>
  <summary>Details</summary>
Motivation: 受近期量子储层计算中噪声可提升性能的发现启发，试图解释噪声为何能成为资源，并从几何角度建立噪声加速量子态分布趋近哈尔分布的机制。

Method: 采用最小1量子比特模型，将振幅阻尼信道与重归一化规则结合，定义纯态空间上的全局扩张非线性映射；理论推导局部面积扩张因子并确定全局扩张阈值；将振幅阻尼与通用门集G3={H,T,CNOT}组合，数值验证趋近哈尔行为的加速效应。

Result: 非幺正噪声在适当参数区域能诱导纯态流形上的有效体积扩张，从而加速最终量子态向哈尔分布的趋近过程。

Conclusion: 噪声在特定条件下可作为未来量子算法的潜在资源，而非仅被视为障碍。
...
TL;DR: 本文通过微观Bloch-Redfield方法重新审视了耗散驱动量子比特-谐振器动力学，发现与标准Lindblad主方程相比，在无驱动时衰减率存在定量差异，在驱动时Redfield耗散器的时间相关与时间无关形式表现出定性差异，并研究了结构化谱密度中驱动对测量诱导弛豫的抑制效应。


<details>
  <summary>Details</summary>
Motivation: 标准的Lindblad主方程使用子系统局域跳跃算符来建模耗散驱动量子比特-谐振器动力学，但当日振子发生明显杂化时，这种近似可能不准确。本文旨在通过更微观的Bloch-Redfield方法，在耦合量子比特-谐振器哈密顿量的本征基下构建耗散，并考虑传输线环境的完整频率依赖开系统描述，以揭示Lindblad近似的局限性。

Method: 采用微观Bloch-Redfield方法，在耦合量子比特-谐振器哈密顿量的本征基下构建耗散，对传输线环境使用完整、频率依赖的开系统描述。对比了无驱动时Lindblad与Bloch-Redfield衰减率的定量差异，以及驱动情况下时间无关Redfield耗散器与其时间依赖推广形式的定性行为差异。此外，研究了结构化谱密度中驱动对测量诱导弛豫的影响，特别是Purcell滤波器的作用。

Result: (1) 无驱动时，Lindblad和Bloch-Redfield衰减率存在定量差异；(2) 驱动情况下，时间无关Redfield耗散器与时间依赖推广形式随驱动强度变化表现出定性不同的行为；(3) 在结构化谱密度中，驱动可以抑制测量诱导的弛豫，即Purcell滤波器的效果。

Conclusion: 标准的Lindblad主方程在量子比特-谐振器明显杂化时可能不够准确，Bloch-Redfield方法能提供更精确的描述。驱动对耗散动力学有显著影响，时间相关与时间无关的Redfield形式在强驱动下行为不同。Purcell滤波器能有效抑制测量诱导的弛豫，验证了结构化环境对耗散工程的重要性。
...
TL;DR: 本书探讨了量子力学中若干通常不被讨论的基础问题，包括微观世界概率的起源、普朗克常数h的本质、概率幅与波函数的性质，提出了量子理论的公理体系，并研究了热浴中经典振子可被解释为量子振子的模型，详细讨论了测量问题。


<details>
  <summary>Details</summary>
Motivation: 作者意图揭示量子力学中那些通常被忽视的基础性问题，如概率起源、h常数的物理本质、波函数的实在性等，试图为量子理论建立更坚实的公理化基础。

Method: 采用公理化方法重新表述量子理论体系，并通过经典振子在热浴中的模型来模拟量子行为，从而探讨量子与经典描述之间的边界。

Result: 提出了一套新的量子理论公理系统，并展示了一个经典振子在特定热力学环境下可以被解释为量子振子的现象。

Conclusion: 量子力学的基础问题（如概率、波函数、测量问题）仍有待深入澄清，所提出的公理体系和经典-量子类比模型为理解这些问题提供了新的视角。
...
TL;DR: 本文发现魔法态蒸馏的渐进速率与开销指数之间不存在稳健的定量关系，并基于逻辑Clifford算子测量提出了一种新的蒸馏协议，该协议在开销指数大于1的情况下仍能实现接近线性的渐进速率。


<details>
  <summary>Details</summary>
Motivation: 揭示魔法态蒸馏中两个关键量——开销指数与渐进蒸馏速率——之间的定量关系在不同 regimes 下是否稳健，填补了当前研究中对于开销指数非零且速率亚线性情形下二者关系的认知空白。

Method: 构建了一族基于逻辑Clifford算子测量进行错误检测的魔法态蒸馏协议，该机制源于近期提出的魔法态培养协议（magic state cultivation protocol）。

Result: 该协议的开销指数大于1（非接近零），但渐进蒸馏速率可以任意接近线性速率（即输出/输入魔法态比例趋近于1）。这表明在亚线性速率 regime 下，蒸馏速率不受开销指数的约束。

Conclusion: 开销指数与渐进蒸馏速率之间的定量关系并非普遍稳健；逻辑Clifford算子测量机制在渐近魔法态蒸馏中具有潜力。
...
TL;DR: 本文提出了一种在门控量子计算机上实现绝热量子计算的新框架，无需模拟含时哈密顿量演化，并基于此框架给出了求解量子线性系统问题（QLSP）的多种最优算法，同时改进了Trotter误差的保真度界限。


<details>
  <summary>Details</summary>
Motivation: 传统绝热量子计算依赖含时哈密顿量演化，在门控量子计算机上模拟这一过程需要高昂的额外开销。作者希望提出替代方案，使得绝热过程能在门控量子计算机上高效实现，而不必承担模拟含时演化的代价。

Method: 提出了一个通用框架，用于推导门控量子计算机上实现绝热过程的"绝热定理"；给出了求解QLSP的多种算法（包括已有算法和随机化版本的离散绝热算法）；在框架中描述了Trotter化的版本，在随机化设定下获得了保真度意义上的Trotter误差界限。

Result: 获得求解QLSP的算法，其条件数缩放达到最优；得到的Trotter误差保真度界限渐近优于标准界限。

Conclusion: 所提出的框架能够统一描述多种绝热替代过程，并在QLSP求解和Trotter误差分析上取得了理论上的最优或更优结果。
...
TL;DR: 本文延续多相互作用世界方法，将其从谐振子系统扩展到含奇异势（库仑势、有限陷阱势）的二维有界系统，通过渐近光滑方法处理势能奇点，并基于动力学算法进行数值模拟。结果表明该方法与标准量子力学中的矩阵Numerov方法一致，为求解更复杂系统提供了可能性。


<details>
  <summary>Details</summary>
Motivation: 将确定性的多相互作用世界方法从谐振子系统推广到具有奇异势（如库仑势、有限陷阱势）的二维有界系统，探索该方法在更一般势能形式下的适用性和有效性。

Method: 采用渐近光滑方法处理势能奇点，基于多相互作用世界方法的动力学算法进行数值模拟，并将结果与标准量子力学中的矩阵Numerov方法做对比验证。

Result: 数值模拟结果与标准量子力学中矩阵Numerov方法得出的结果一致，证明了该方法在求解含奇异势的有界系统时的有效性。

Conclusion: 多相互作用世界方法可以成功扩展至二维含奇异势的有界系统，为求解更复杂的量子系统提供了新途径。
...
TL;DR: 本文提出一个三阶段量子基态制备框架：先用DMRG获得矩阵乘积态（MPS），再通过无优化的矩阵乘积解纠缠器（MPD）编码电路将其加载到量子寄存器，最后用概率虚时演化（PITE）抑制剩余误差。MPD编码过程中间态的中心键施密特秩随层数逻辑斯蒂增长，拐点L*标志着高效编码边界；超过该点后，达到目标保真度所需的MPD层数经验标度为O(N⁵ log(N/ε))。为避免深层编码，在L*处停止编码并用PITE处理剩余激发态分量，PITE的线性调度由基态能量、有效能隙和DMRG估计的参考重叠确定。数值实验表明该框架避免了极深编码电路并显著抑制了PITE的后选择开销，为量子模拟中的基态制备提供了一条实用混合路线。


<details>
  <summary>Details</summary>
Motivation: 量子模拟中，制备的初态与真实基态的重叠度显著影响后续量子算法的整体成本，因此高效制备高保真度基态是核心挑战。现有方法中，直接通过变分量子电路制备基态面临优化困难和电路深度大的问题；而基于虚时演化的方法则存在严重的后选择开销。作者旨在结合经典DMRG的预处理能力与量子处理器的演化优势，设计一个可实际部署的混合框架，避免深层编码电路并控制后选择成本。

Method: 三阶段混合框架：(1) 经典预计算阶段，用DMRG获得N-站点系统的MPS表示；(2) 无优化MPS编码阶段，通过矩阵乘积解纠缠器（MPD）电路将MPS直接加载到N-量子比特寄存器，无需变分优化；(3) 误差抑制阶段，使用概率虚时演化（PITE）压制剩余激发态分量，PITE的线性调度参数（步长、演化总时长）根据DMRG给出的基态能量、有效能隙和参考重叠确定。作者还分析了MPD编码过程中中心键施密特秩随层数的逻辑斯蒂增长行为，定义了高效编码的拐点L*。

Result: (1) MPD编码过程中间态的施密特秩随编码层数呈逻辑斯蒂增长，存在拐点L*；(2) 超过L*后，达到目标保真度所需的额外MPD层数经验标度为O(N⁵ log(N/ε))；(3) 在自旋-1/2交错场海森堡链的数值实验中，该框架避免了极深编码电路，且显著抑制了PITE方法固有的大后选择开销；(4) 在编码深度和后选择成本之间取得了有效平衡。

Conclusion: 通过结合经典DMRG预处理、无优化MPS编码和确定性调度的PITE，本文提出的三阶段框架为量子模拟中的基态制备提供了一条实用的混合路线。该框架的核心优势在于：(a) 编码电路无需变分优化，避免了经典优化的困难；(b) 在拐点L*处停止编码，避免了编码效率急剧下降带来的深层电路问题；(c) 利用DMRG提供的经典信息确定性调度PITE，避免了PITE中通常需要的增量式试探。该框架在近期含噪量子设备上具有实际可行性。
...
TL;DR: 本文提出利用Koopman-von Neumann (KvN)表示将经典分子动力学的Green-Kubo输运系数计算转化为量子算法的读出问题。在有限网格上的数值基准表明，离散化误差随格点数呈幂律下降，等价于随寄存器大小指数下降，目标精度ε需要n_z=O(log(1/ε))量子比特。通过量子相位估计读取输运系数，结合最大似然振幅估计可将统计估计从N_queries^{-1/2}提升至接近N_queries^{-1}标度。NVE传播器单步可用O(n^2)个CX门构建，NVT传播器的Nosé-Hoover摩擦项按中心差分Pauli分解实现，标度为O(n_ξ n_p 2^{n_p})。


<details>
  <summary>Details</summary>
Motivation: 将经典分子动力学中的输运系数（如扩散系数、热导率等）计算映射到量子算法框架，利用量子计算的潜在优势提升计算效率，为实际分子模拟中的输运系数计算提供量子算法路径。

Method: (1) 利用KvN表示将经典NVE和Nosé-Hoover型NVT动力学编码为Hilbert空间上的酉演化；(2) 在有限网格上离散化，分析离散化误差与格点数的标度关系；(3) 将通量激发态输入QPE，通过测量QPE辅助寄存器全零态的概率P_0获得Bartlett窗Green-Kubo积分；(4) 采用最大似然振幅估计提升统计估计精度；(5) 进行电路资源分析，给出NVE和NVT传播器的门复杂度。

Result: (1) 离散化误差随格点数N_z呈幂律下降，随量子比特数n_z呈指数下降；(2) 目标精度ε需n_z=O(log(1/ε))量子比特；(3) 最大似然振幅估计使统计估计标度接近N_queries^{-1}；(4) NVE传播器单步需O(n^2) CX门；(5) NVT传播器摩擦项标度为O(n_ξ n_p 2^{n_p})。

Conclusion: 该框架是将量子算法原理转化为实际分子模拟中输运系数计算的具体步骤，展示了量子计算在经典分子动力学输运性质计算中的可行性和潜在优势。
...
TL;DR: 本文构建了一个用于有限温度正则系综分子动力学的量子电路框架，结合Langevin恒温器，将经典核相空间分布编码为Koopman–von Neumann (KvN)波函数，通过分解Liouville流、动量摩擦和动量扩散为独立电路模块，并在H₂分子上验证了从非平衡态到正则KvN态的弛豫及振动态密度与过渡态理论速率常数的读出。


<details>
  <summary>Details</summary>
Motivation: 在量子计算机上实现量子-经典混合分子动力学，特别是有限温度下正则系综的模拟，需要一种电路级别的协议来连接正则态制备与物理性质计算，当前缺乏这样的系统性框架。

Method: 将经典核相空间分布编码为KvN波函数；将Langevin动力学分解为哈密顿Liouville流（幺正演化）、动量摩擦（对称化动量空间膨胀）和动量扩散（基于PITE的余弦滤波器）三个电路模块；通过解析量化PITE余弦滤波器替代高斯扩散核引入的温度偏差，并给出内禀温度修正。

Result: 在H₂分子上展示了从非平衡相空间分布到正则KvN态的弛豫过程；实现了两种互补读出：量子相位估计提取H-H伸缩振动的振动态密度，以及正则系综下的过渡态理论速率常数计算。

Conclusion: 该工作在最小分子系统中演示了连接Langevin正则态制备与物理性质计算的电路级协议，为量子计算机上的量子-经典混合分子动力学提供了具体可行的步骤。
...
### [69] [Programmable Dissipation via Partial Quantum Error Correction](https://arxiv.org/abs/2605.30217)
*Sameer Dambal,Michael AD Taylor,Yu Zhang*

Main category: quant-ph

TL;DR: 本文提出将逻辑噪声转化为可控资源：通过部分量子纠错，利用纠错周期和解码器/恢复随机化编程，直接编译目标耗散动力学，无需辅助比特编码浴自由度，实现开放量子系统的高效模拟。


<details>
  <summary>Details</summary>
Motivation: 传统上将噪声视为量子信息处理的敌人，但在开放量子动力学中，耗散是目标物理的一部分，这与容错架构抑制退相干的目标产生矛盾。本文旨在化解这一矛盾，将逻辑噪声转化为可校准的资源。

Method: 将纠错周期视为可编程原语：一轮容错纠错实现逻辑上的完全正迹保持映射；通过解码器/恢复随机化生成可控的逻辑通道族，其凸混合实现Kraus通道混合。无需显式的辅助量子比特编码浴自由度，直接编译目标耗散为有效逻辑动力学。

Result: 推导了多步模拟的精度判据：选择码距离使未受控的逻辑误差仅占每步目标耗散的一小部分，而非被压制到任意小的封闭系统容差以下。部分量子纠错将容错结构重新用于塑形耗散。

Conclusion: 该方法提供了一种资源高效的开放量子系统模拟路径，通过重新利用容错结构来构造耗散，而非完全抑制噪声。
...
### [70] [Non-Abelian Mixer for QAOA on Hybrid Oscillator-Qubit Quantum Processors](https://arxiv.org/abs/2605.30234)
*Thinh Le,Hansika Weerasena,Jianqing Liu*

Main category: quant-ph

TL;DR: 本文针对混合振子-量子比特（CV-DV）量子处理器，提出了一种硬件原生的非阿贝尔混频器用于QAOA算法，并在Max-Cut问题上验证了其相比标准横向场混频器在近似比和最优解概率上的显著优势。


<details>
  <summary>Details</summary>
Motivation: 混合振子-量子比特量子处理器已实现通用控制，但该平台上量子算法的开发仍处于早期阶段。尽管QAOA在CV和DV系统中分别得到广泛研究，但在混合CV-DV场景下的发展仍然有限。

Method: 提出一种硬件原生的非阿贝尔混频器用于混合CV-DV量子处理器上的QAOA，并开发了针对Max-Cut问题的混合ansatz。在未加权Erdős-Rényi图上进行仿真评估，与标准横向场混频器进行对比。

Result: 在所有图尺寸和Fock截断下，所提出的非阿贝尔混频器在期望解质量和最优解采样概率上均一致优于横向场混频器。

Conclusion: 该非阿贝尔混频器是混合振子-量子比特平台上QAOA的一个有前景的构建模块。
...
### [71] [Indefinite Causal Order Reverses the Real-Complex Hierarchy](https://arxiv.org/abs/2605.30238)
*Jacopo Surace,Shintaro Minagawa,Ravi Kunjwal*

Main category: quant-ph

TL;DR: 本文研究对称性约束下的量子过程矩阵框架，发现有限酉对称性不产生新关联，而实量子理论在不定因果序下比复量子理论实现更多过程关联。


<details>
  <summary>Details</summary>
Motivation: 探索因果关系是否能像其他物理性质一样具有量子不确定性，以及对称性约束如何影响这种可能性。

Method: 在过程矩阵框架中引入对称性约束（如缺失参考系、超选择约束、反酉对称等），分析这些约束下允许的过程锥扩展情况，并证明一个严格二分法。

Result: 对于任何有限酉对称性，不会产生真正的新关联；但对于实量子理论（反酉对称），会产生新关联。在不定因果序下，实量子理论比复量子理论实现更丰富的过程关联。

Conclusion: 在固定因果序下复量子理论更丰富，但在不定因果序下实量子理论更丰富，这一层次关系发生反转。
...
### [72] [Quantum optimization beyond QUBO for industrial logistics and scheduling](https://arxiv.org/abs/2605.30252)
*Juan F. R. Hernandez,Pavle Nikacevic,Enrique Solano,Chinonso Onah,Agneev Guin,Arne-Christian Voigt,Archismita Dalal*

Main category: quant-ph

TL;DR: 本文研究了高阶无约束二元优化（HUBO）公式在工业调度和运输路由问题中的量子优化应用，与标准QUBO形式相比，HUBO虽能减少量子比特需求，但引入了更高阶的相互作用项，增加了电路深度，当前更适合混合量子-经典工作流和早期容错量子硬件。


<details>
  <summary>Details</summary>
Motivation: 工业调度和运输路由问题的日益复杂性促使研究者探索替代优化公式和计算范式，特别是量子优化方法。

Method: 选取三个代表性的物流和制造用例，分别构建HUBO问题公式；与QUBO编码进行对比；使用经典求解器验证；对小规模路由问题实例采用偏置场数字化反绝热量子优化进行经典仿真；对大规模有容量车辆路径问题进行资源与可扩展性分析。

Result: HUBO公式通过紧凑二元编码减少了量子比特需求，但引入了高阶相互作用项，增加了电路深度；在量子比特扩展性上优于QUBO，但受限于门保真度、相干时间和电路深度。

Conclusion: HUBO公式在实际量子硬件上的实现受约束，混合量子-经典工作流和早期容错量子硬件是其最可行的实践场景。
...
### [73] [Quantum Synchronization of Fock States](https://arxiv.org/abs/2605.30271)
*Fabian Hassler,David Scheer,Samah Saquaque,Steven Kim*

Main category: quant-ph

TL;DR: 本文展示了玻色模式在福克态极限环下的量子同步现象，该极限环呈现负Wigner函数的非经典稳态，并可通过外部驱动实现阿诺德舌头区域内的相位锁定。研究将同步视为动力学特性，发现其与相位滑移的指数抑制密切相关，并提出了从林德布拉德时间演化中提取相位滑移率的新方法。


<details>
  <summary>Details</summary>
Motivation: 将经典系统中的同步现象拓展至量子领域，探索非经典态（如负Wigner函数的福克态极限环）的同步行为及其动力学机制。

Method: 通过外部驱动对呈现负Wigner函数的福克态极限环进行相位锁定，分析阿诺德舌头区域内的同步行为；基于林德布拉德主方程时间演化，提出提取相位滑移率的新方法。

Result: 实现了非经典福克态极限环的量子同步，观察到同步与相位滑移概率呈指数下降相关联。

Conclusion: 同步是量子系统的动力学属性，非经典态亦可实现同步，为理解和操控非经典同步动力学开辟新路径。
...
TL;DR: 本文针对基于Wave Matrix Lindbladization (WML)算法的Lindbladian模拟，建立了改进的样本复杂度界。推导了非渐近样本复杂度上界 $n_d^*(t,\varepsilon) \le \left( \frac{2d+3}{8} \right) \|L\|_\infty^2 \left( \frac{t^2}{\varepsilon} \right)$，改进了此前 $O(d^2 t^2/\varepsilon)$ 的结果。当 $\|L\|_\infty^2 = O(1/d)$ 时维度开销可完全避免，典型样本复杂度为 $O(t^2/\varepsilon)$；最坏情况下WML需要 $\Omega(d t^2/\varepsilon)$ 样本。揭示了典型与对抗样本复杂度之间的鲜明二分性。


<details>
  <summary>Details</summary>
Motivation: 为样本基Lindbladian模拟建立更紧的样本复杂度界，改进先前工作中对维度依赖的过高估计，并理解不同场景下样本复杂度的本质差异。

Method: 基于Wave Matrix Lindbladization (WML)算法，通过精细分析推导非渐近样本复杂度上界，并构造显式反例证明下界。区分了典型情况（随机Lindblad算子）与最坏情况（秩一Lindblad算子）的样本复杂度。

Result: 得到了 $n_d^*(t,\varepsilon) \le \left( \frac{2d+3}{8} \right) \|L\|_\infty^2 \left( \frac{t^2}{\varepsilon} \right)$ 的上界，维度依赖从 $O(d^2)$ 降至 $O(d)$；典型情况下样本复杂度为 $O(t^2/\varepsilon)$，最坏情况下下界为 $\Omega(d t^2/\varepsilon)$。

Conclusion: 揭示了Lindbladian模拟中样本复杂度的典型-对抗二分性，为样本基量子算法奠定了更坚实的理论基础。
...
### [75] [Quantum Desynchronization of Limit Cycles](https://arxiv.org/abs/2605.30302)
*Hans Christiansen,Jens Paaske*

Main category: quant-ph

TL;DR: 本文通过Keldysh路径积分框架研究连续变量量子系统的同步现象，发现量子相位滑移会破坏相位锁定，并分析了非马尔可夫效应对极限环同步的影响，以超导谐振器通过偏压双量子点耦合为例进行说明。


<details>
  <summary>Details</summary>
Motivation: 经典物理学中弱耦合自持振荡器可自发锁定相位，但噪声引起的相位滑移会破坏经典同步。本文旨在探索连续变量量子系统中是否存在类似的量子相位滑移机制，以及它们如何影响量子系统的同步行为。

Method: 采用Keldysh路径积分形式化描述极限环，分析相位动力学；研究量子相位滑移对相位锁定的影响；处理非马尔可夫效应对极限环同步的作用；以电压偏置双量子点耦合的超导谐振器为例进行具体说明。

Result: 在连续变量量子系统中，尽管存在强相位关联，量子相位滑移仍然会破坏实际的相位锁定，类似于经典噪声引起的相位滑移破坏同步。非马尔可夫效应对极限环同步也有重要影响。

Conclusion: 量子相位滑移是导致连续变量量子系统同步退化的关键机制，这一发现将经典同步理论中噪声破坏同步的概念推广到量子领域，并揭示了非马尔可夫效应在量子同步中的重要作用。
